...
namespace YSCMS.Common{
	...
	public class Public {
		...
		/// <summary>
		/// 得到SQL语句的SiID;getstr = " and SiteID='" + YSCMS.Global.Current.SiteID + "'"
		/// </summary>
		/// <returns></returns>
		public static string getSessionStr() {
			string getstr = "";
			if (YSCMS.Global.Current.SiteID != "0") { getstr = " and SiteID='" + YSCMS.Global.Current.SiteID + "'"; }
			return getstr;
		}

		/// <summary>
		/// 得到频道ID; and ChannelID='" + YSCMS.Global.Current.SiteID + "'
		/// </summary>
		/// <returns></returns>
		public static string getCHStr() {
			string getstr = "";
			if (YSCMS.Global.Current.SiteID != "0") { getstr = " and ChannelID='" + YSCMS.Global.Current.SiteID + "'"; }
			return getstr;
		}
		...
		public static void savePublicLogFiles(string item, string errorContent, string username) {
			StreamWriter sw = null;
			DateTime date = DateTime.Now;
			string FileName = YSCMS.Config.UIConfig.Logfilename;
			try {
				if (HttpContext.Current == null) {
		...
		public static List<string> GetFTPQueue() {
			string ftpQueueFile = Path.Combine(ServerInfo.GetRootPath(), YSCMS.Config.UIConfig.FTPQueueFile);
