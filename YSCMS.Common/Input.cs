...
namespace YSCMS.Common
{
	public class Input {
		...
		public static string LostVoteStr(string Str) {
			...
			if (Str != null) {
				if (Str != string.Empty) {
					string Pattern = "\\[YS:unLoop\\/*[^<>]*\\[\\/YS:unLoop\\]";
					Re_Str = Regex.Replace(Str, Pattern, "");
				}
			}
			return Re_Str;
		...
		public static string GetSubString(string Str, int Num) {
			string titlemore = YSCMS.Config.UIConfig.titlemore;
			string lastchar = "";
			if (titlemore == "1") {
				lastchar = "...";
			}
		...
		public static string isPicStr(string inputStr) {
			string dimm = YSCMS.Config.UIConfig.dirDumm;
			...
			if (inputStr == string.Empty && inputStr == null) {
				...
				if (inputStr.IndexOf(".") > -1) {
					...
					if (PType == 1) {
						gStr = "<img src=\"" + dimm + inputStr.ToLower().Replace("{@dirfile}", YSCMS.Config.UIConfig.dirFile) + "\" border=\"0\" />";
					}
					else {
						gStr = inputStr;
					}
		...
		public static string AutoSplitPage(string content, int pageSize) {
			string pagedContent = "";
			string split_str = "[YS:Page]";
			content = content.Replace("[YS:Page]", "");
