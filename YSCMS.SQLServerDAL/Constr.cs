...
namespace YSCMS.SQLServerDAL
{
	public class Constr : DbBase, IConstr
	{
		...
		{
			string Sql = "select ConstrTF from " + Pre + "sys_PramUser where SiteID = '" + YSCMS.Global.Current.SiteID + "'";
		...
		public DataTable GetPage1(int PageIndex, int PageSize, out int RecordCount, out int PageCount, params SQLConditionInfo[] SqlCondition)
		{
			...
			string Condition = "(select DISTINCT UserNum from " + Pre + "user_Constr where substring(Contrflg,3,1) = '1' and SiteID='" + YSCMS.Global.Current.SiteID + "') UserNum1";
        ...
        {
            //string Sql = "Select ConstrPayName,PCId From " + Pre + "sys_ParmConstr where SiteID='" + YSCMS.Global.Current.SiteID + "'";
            //return DbHelper.ExecuteTable(CommandType.Text, Sql, null);
            return null;
        }
		...
		public int Add4(string NewsID, int gPoint, int iPoint, int Money1, DateTime CreatTime1, string UserNum, string content4)
		{
			...
			string Sql = "insert into " + Pre + "User_Ghistory(GhID,ghtype,Gpoint,iPoint,Money,CreatTime,UserNUM,gtype,content,siteID) values(@NewsID,1,@gPoint,@iPoint,@Money1,@CreatTime1,@UserNum,4,@content4,'" + YSCMS.Global.Current.SiteID + "')";
			return Convert.ToInt32(DbHelper.ExecuteNonQuery(CommandType.Text, Sql, param));
		}
		...
		public int GetParmConstrNum(string UserNum)
		{
			SqlParameter param = new SqlParameter("@UserNum", UserNum);
			int intflg = 0;
			string Sql = "select ParmConstrNum from " + Pre + "sys_user where UserNum=@UserNum and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			DataTable dt = DbHelper.ExecuteTable(CommandType.Text, Sql, param);
			if (dt != null)
			{
				if (dt.Rows.Count > 0)
		...
		public int Add5(string UserNum1, int ParmConstrNums, DateTime payTime, string constrPayID)
		{
			...
			string Sql = "insert into " + Pre + "user_constrPay(userNum,Money,payTime,constrPayID,SiteID,PayAdmin) values(@userNum,@ParmConstrNums,@payTime,@constrPayID,'" + YSCMS.Global.Current.SiteID + "','" + YSCMS.Global.Current.UserName + "')";
			return Convert.ToInt32(DbHelper.ExecuteNonQuery(CommandType.Text, Sql, param));
		}
		...
		public int Add6(string PCId, string ConstrPayName, string gpoint, string ipoint, int moneys1, string Gunit)
		{
			...
			param[5].Value = Gunit;
			string Sql = "insert into " + Pre + "sys_ParmConstr(PCId,ConstrPayName,gPoint,iPoint,money,Gunit,SiteID) values(@PCId,@ConstrPayName,@gpoint,@ipoint,@moneys1,@Gunit,'" + YSCMS.Global.Current.SiteID + "')";
			return Convert.ToInt32(DbHelper.ExecuteNonQuery(CommandType.Text, Sql, param));
		}
