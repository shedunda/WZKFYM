...
namespace YSCMS.SQLServerDAL
{
    ...
    public partial class NewsSpecial : DbBase, INewsSpecial
    {
        ...
        /// 增加一条数据
        /// </summary>
        public string Add(YSCMS.Model.NewsSpecial model)
        {
            int result = 0;
            string SpecialID = "";
            string checkSql = "";
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(YSCMS.Model.NewsSpecial model)
        {
            int result = 0;
            string checkSql = "";
            int recordCount = 0;
        ...
        /// 得到一个对象实体
        /// </summary>
        public YSCMS.Model.NewsSpecial GetModel(string SpecialID)
        {
            ...
            strSql.Append(" where SpecialID=@SpecialID and SiteID='" + YSCMS.Global.Current.SiteID + "'");
            SqlParameter[] parameters = {
					new SqlParameter("@SpecialID", SqlDbType.NVarChar,12)			};
            parameters[0].Value = SpecialID;

            YSCMS.Model.NewsSpecial model = new YSCMS.Model.NewsSpecial();
        ...
        public DataTable GetChildList(string classid)
        {
            string str_Sql = "Select Id,SpecialID,SpecialCName,CreatTime,isLock From " + Pre + "news_special " +
                             "Where isRecyle=0 and ParentID='" + classid + "' and SiteID='" + YSCMS.Global.Current.SiteID + "'";
        ...
        public int SetLock(string specialID)
        {
            ...
            if (specialID.IndexOf(",") > -1)
            {
                ...
                {
                    SqlArray.Add("update " + Pre + "news_special set isLock=(case isLock when 1 then 0 when 0 then 1 end) where SpecialID=" + id + " and SiteID=" + YSCMS.Global.Current.SiteID);
                }
            }
            else
            {
                SqlArray.Add("update " + Pre + "news_special set isLock=(case isLock when 1 then 0 when 0 then 1 end) where SpecialID=" + specialID + " and SiteID=" + YSCMS.Global.Current.SiteID);
            }
        ...
        public int SetRecyle(string specialId)
        {
            ...
            if (specialId.IndexOf(",") > -1)
            {
                ...
                {
                    sqlArray.Add("update " + Pre + "news_special set isRecyle=(case isRecyle when 1 then 0 when 0 then 1 end) where SpecialID=" + id.Replace("'", "") + " and SiteID=" + YSCMS.Global.Current.SiteID);
                }
            }
            else
            {
                sqlArray.Add("update " + Pre + "news_special set isRecyle=(case isRecyle when 1 then 0 when 0 then 1 end) where SpecialID=" + specialId + " and SiteID=" + YSCMS.Global.Current.SiteID);
            }
            return DbHelper.ExecuteSqlTran(sqlArray);
        }
