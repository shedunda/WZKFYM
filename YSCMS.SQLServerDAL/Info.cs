...
namespace YSCMS.SQLServerDAL {
	public class Info : DbBase, IInfo {
		...
		public int Delete(string FID) {
			...
			string Sql = "delete " + Pre + "API_Faviate where FID=@FID And UserNum='" + YSCMS.Global.Current.UserNum + "'";
			return Convert.ToInt32(DbHelper.ExecuteNonQuery(CommandType.Text, Sql, param));
		}
		...
		public bool addTo(string NewsID, int ChID) {
			...
			param[1].Value = ChID;
			string gsql = "select count(id) from " + Pre + "API_Faviate where FID=@NewsID and ChID=@ChID And UserNum='" + YSCMS.Global.Current.UserNum + "'";
			int i_Count = Convert.ToInt32(DbHelper.ExecuteScalar(CommandType.Text, gsql, param));
			if (i_Count == 0) {
				string Sql = "insert " + Pre + "API_Faviate(FID,UserNum,CreatTime,APIID,DataLib,ChID) values(@NewsID,'" + YSCMS.Global.Current.UserNum + "','" + DateTime.Now + "','0','',@ChID)";
		...
		public DataTable GetPagepoi(string typep, string UM, string sle_NUM, string SiteID, string UserNum, int PageIndex, int PageSize, out int RecordCount, out int PageCount, params SQLConditionInfo[] SqlCondition) {
			...
			if (SiteID != "" && SiteID != "0" && SiteID != null) {
				if (YSCMS.Global.Current.SiteID == "0") {
					...
					siteID1 = " and SiteID='" + YSCMS.Global.Current.SiteID + "'";
				}
			}
			else {
				siteID1 = " and SiteID='" + YSCMS.Global.Current.SiteID + "'";
		...
		public DataTable StartUserC() {
			string Str_FriSql = "Select UserNum,Email From " + Pre + "sys_User where UserNum='" + YSCMS.Global.Current.UserNum + "'";
		...
		public int SaveLink(string Str_Class, string Str_Name, string Str_Type, string Str_Url, string Str_Content, string Str_PicUrl, string Str_Author, string Str_Mail, string Str_ContentFor) {
			...
			string Str_InSql = "Insert into " + Pre + "friend_link (ClassID,Name,Type,Url,Content,PicUrl,Author,Mail,ContentFor,isAdmin,Lock,IsUser,SiteID) Values(@Str_Class,@Str_Name,@Str_Type,@Str_Url,@Str_Content,@Str_PicUrl,@Str_Author,@Str_Mail,@Str_ContentFor,0,1,1,'" + YSCMS.Global.Current.SiteID + "')";
			return DbHelper.ExecuteNonQuery(CommandType.Text, Str_InSql, param);
		}

		public void delf(string id) {
			SqlParameter param = new SqlParameter("@ID", id);
			string Str_InSql = "delete from " + Pre + "friend_link where Author='" + YSCMS.Global.Current.UserNum + "' and ID=@ID";
