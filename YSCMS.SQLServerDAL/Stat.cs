...
namespace YSCMS.SQLServerDAL
{
	...
	{
		string isDataBase = YSCMS.Config.UIConfig.indeData;
		...
		public string Str_ClassSql(string id)
		{
			...
			string Sql = "Select classname From " + Pre + "stat_class where statid=@id and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			return Convert.ToString(DbHelper.ExecuteScalar(CommandType.Text, Sql, param));
		}
		public bool Str_classSql(string ID)
		{
			SqlParameter param = new SqlParameter("@ID", ID);
			string Sql = "Delete From " + Pre + "stat_class where statid=@ID and SiteID='" + YSCMS.Global.Current.SiteID + "'";
		...
		{
			SqlParameter param = new SqlParameter("@ID", ID);
			string Sql = "Delete From " + Pre + "stat_Info where classid = @ID and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			int i = DbHelper.ExecuteNonQuery(CommandType.Text, Sql, param);
			if (i == 0)
			{
				return false;
		...
		{
			SqlParameter param = new SqlParameter("@ID", ID);
			string Sql = "Delete From " + Pre + "stat_Content where classid = @ID and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			int i = DbHelper.ExecuteNonQuery(CommandType.Text, Sql, param);
			if (i == 0)
			{
				return false;
		...
		public void Str_InSql(YSCMS.Model.stat_Param sp)
		{
		...
		private SqlParameter[] Getstat_Param(YSCMS.Model.stat_Param sp)
		{
		...
		{
			int intnum = 20;
			string Sql = "Select pageNum From " + Pre + "stat_Param where SiteID='" + YSCMS.Global.Current.SiteID + "'";//取得参数设置中的每页显示数
		...
		{
			string Sql = "Delete From " + Pre + "stat_class where statid in ('" + CheckboxArray + "') and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
		}
		public void Str_statInfo_Sql_1(string CheckboxArray)
		{
			string Sql = "Delete From " + Pre + "stat_Info where classid in ('" + CheckboxArray + "') and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
		}
		public void Str_statContent_Sql_1(string CheckboxArray)
		{
			string Sql = "Delete From " + Pre + "stat_Content where classid in ('" + CheckboxArray + "') and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
		}
		public bool Str_StatSql()
		{
			string Sql = "Delete From " + Pre + "stat_class where SiteID='" + YSCMS.Global.Current.SiteID + "'";
		...
		{
			string Sql = "Delete From " + Pre + "stat_Info where SiteID='" + YSCMS.Global.Current.SiteID + "'";
			int i = DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
			if (i == 0)
			{
				return false;
		...
		{
			string Sql = "Delete From " + Pre + "stat_Content where SiteID='" + YSCMS.Global.Current.SiteID + "'";
			int i = DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
			if (i == 0)
			{
				return false;
		...
		public int sel_1(string Str_statid)
		{
			...
			string Sql = "Select count(statid) From " + Pre + "stat_class where statid = @statid and SiteID='" + YSCMS.Global.Current.SiteID + "'";
		...
		public int Str_CheckSql(string Str_Classname)
		{
			...
			string Sql = "Select count(classname) From " + Pre + "stat_class Where classname=@Classname and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			return (int)DbHelper.ExecuteScalar(CommandType.Text, Sql, param);
		}
		...
		{
			string Sql = "Update " + Pre + "stat_class set classname='" + Str_ClassnameE + "' where statid='" + id + "' and SiteID='" + YSCMS.Global.Current.SiteID + "'";
			return DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
		}
		...
		public void Add(stat_Info info)
		{
			...
			string Sql = "insert into " + Pre + "stat_Info(vyear,vmonth,vday,vhour,vtime,vweek,vip,vwhere,vwheref,vcome,vpage,vsoft,vOS,vwidth,classid,SiteID) values(@str_ys_year,@str_ys_month,@str_ys_day,@str_ys_hour,getdate(),@str_ys_week,@str_ys_ip,@str_ys_country,@str_ys_city,@str_ys_come,@str_ys_page,@str_ys_soft,@str_ys_Os,@str_ys_width,@statid,@SiteID)";
		...
		private SqlParameter[] Getstat_Info(stat_Info info)
		{
			...
			parm[0] = new SqlParameter("@str_ys_year", SqlDbType.Int, 4);
			parm[0].Value = info.vyear;
			parm[1] = new SqlParameter("@str_ys_month", SqlDbType.Int, 4);
			parm[1].Value = info.vmonth;
			parm[2] = new SqlParameter("@str_ys_day", SqlDbType.Int, 4);
			parm[2].Value = info.vday;
			parm[3] = new SqlParameter("@str_ys_hour", SqlDbType.Int, 4);
			parm[3].Value = info.vhour;
			parm[4] = new SqlParameter("@str_ys_time", SqlDbType.NVarChar, 4);
			parm[4].Value = info.vtime;
			parm[5] = new SqlParameter("@str_ys_week", SqlDbType.Int, 4);
			parm[5].Value = info.vweek;
			parm[6] = new SqlParameter("@str_ys_ip", SqlDbType.NVarChar, 50);
			parm[6].Value = info.vip;
			parm[7] = new SqlParameter("@str_ys_country", SqlDbType.NVarChar, 250);
			parm[7].Value = info.vwhere;
			parm[8] = new SqlParameter("@str_ys_city", SqlDbType.NVarChar, 50);
			parm[8].Value = info.vwheref;
			parm[9] = new SqlParameter("@str_ys_come", SqlDbType.NVarChar, 250);
			parm[9].Value = info.vcome;
			parm[10] = new SqlParameter("@str_ys_page", SqlDbType.NVarChar, 250);
			parm[10].Value = info.vpage;
			parm[11] = new SqlParameter("@str_ys_soft", SqlDbType.NVarChar, 50);
			parm[11].Value = info.vsoft;
			parm[12] = new SqlParameter("@str_ys_Os", SqlDbType.NVarChar, 50);
			parm[12].Value = info.vOS;
			parm[13] = new SqlParameter("@str_ys_width", SqlDbType.Int, 4);
		...
		public void Update(int str_ys_today, int str_ys_yesterday, string str_ys_content_data, int str_ys_all, int str_ys_heigh, string str_ys_heightime, string strclassid, string siteID, string strclassids)
		{
			string Sql = "Update " + Pre + "stat_Content set today=" + str_ys_today + ",yesterday=" + str_ys_yesterday + ",vdate='" + str_ys_content_data + "',vtop=" + str_ys_all + ",vhigh=" + str_ys_heigh + ",vhightime='" + str_ys_heightime + "',classid='" + strclassid + "',SiteID='" + siteID + "' where classid='" + strclassid + "'";
