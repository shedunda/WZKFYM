namespace YSCMS.Publish
{
    ...
    public class CommonData
    {
        ...
        public static IPublish DalPublish = DataAccess.CreatePublish();
        ...
        private static News _setNewsInfos(IDataReader rd)
        {
            News news = new News {
                ...
                NewsID = Convert.ToString(rd["NewsID"]),
                NewsType = Convert.ToByte(rd["NewsType"]),
                OrderID = Convert.ToByte(rd["OrderID"]),
                NewsTitle = Convert.ToString(rd["NewsTitle"])
            };
            if (rd["sNewsTitle"] == DBNull.Value)
            {
                news.sNewsTitle = "";
