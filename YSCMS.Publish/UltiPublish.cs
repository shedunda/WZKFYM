namespace YSCMS.Publish
{
    ...
    public class UltiPublish
    {
        ...
        private int ys_ClassFlag;
        private bool ys_isClassIndex;
        private bool ys_isProgressBar;
        private bool ys_isPubClass;
        private bool ys_isPubindex;
        private bool ys_isPubIsPage;
        private bool ys_isPubNews;
        private bool ys_isPubSpecial;
        private int ys_newsFlag;
        private int ys_specialFlag;
        private string ys_strClassIsPageParam;
        private string ys_strClassParams = string.Empty;
        private string ys_strNewsParams = string.Empty;
        private string ys_strSpecialParams = string.Empty;
        ...
        {
            this.ys_isProgressBar = isProgressBar;
        ...
        public void BeganToPublish()
        {
            ...
            HProgressBar.Start();
            if (this.ys_isPubindex)
            {
                this.ultiPublishIndex();
            }
            if (this.ys_isPubClass)
            {
                this.ultiPublishClass();
            }
            if (this.ys_isPubSpecial)
            {
                this.ultiPublishSpecial();
            }
            if (this.ys_isPubIsPage)
            {
                this.ultiPublishIsPage();
            }
            if (this.ys_isPubNews)
            {
            ...
            if (!(((this.ys_isPubindex || this.ys_isPubNews) || (this.ys_isPubClass || this.ys_isPubSpecial)) || this.ys_isPubIsPage))
            {
        ...
        {
            if (this.ys_strClassParams.IndexOf("#") >= 0)
            {
        ...
        {
            if (this.ys_strClassParams != null)
            {
                string classid = "";
                string[] strArray = this.ys_strClassParams.Split(new char[] { '$' });
        ...
        protected IDataReader GetNewsClasses()
        {
            ...
            bool isdesc = false;
            if (this.ys_strNewsParams.IndexOf("#") >= 0)
            {
                unpublish = true;
                this.ys_strNewsParams = this.ys_strNewsParams.Replace("#", "");
            }
            if (this.ys_strNewsParams.IndexOf("&") >= 0)
            {
                isdesc = true;
            ...
            string classid = "";
            string[] strArray = this.ys_strNewsParams.Split(new char[] { '$' });
        ...
        {
            DateTime startTm = Convert.ToDateTime(this.ys_strNewsParams.Split(new char[] { '$' })[0]);
            DateTime endTm = Convert.ToDateTime(this.ys_strNewsParams.Split(new char[] { '$' })[1]);
            return CommonData.DalPublish.GetPublishNewsByTime(startTm, endTm, out this.nNewsCount);
        }

        protected IDataReader GetNewsId()
        {
            int minid = Convert.ToInt32(this.ys_strNewsParams.Split(new char[] { '$' })[0]);
            int maxid = Convert.ToInt32(this.ys_strNewsParams.Split(new char[] { '$' })[1]);
        ...
        {
            int num;
            IDataReader reader = CommonData.DalPublish.GetPublishNewsLast(Convert.ToInt32(this.ys_strNewsParams), false, out num);
            this.nNewsCount = Convert.ToInt32(this.ys_strNewsParams);
        ...
        {
            int num;
            IDataReader reader = CommonData.DalPublish.GetPublishNewsLast(Convert.ToInt32(this.ys_strNewsParams), true, out num);
            this.nNewsCount = Convert.ToInt32(this.ys_strNewsParams);
            if (num < this.nNewsCount)
            {
                this.nNewsCount = num;
        ...
        public string GetPagelist(string Numstr, int i, string getFileName, string getFileEXName, string postResult_color, string postResult_css, int n, string ID, string ContentType, int isPop, string CurrentSytle, string OtherStyle)
        {
            ...
            switch (Numstr)
            {
                ...
                case "4":
                {
                    ...
                    {
                        postResult_css = " class=\"yuesi_pagebox\"";
                        CurrentSytle = " class=\"yuesi_pagebox_num_nonce\"";
                        OtherStyle = " class=\"yuesi_pagebox_num\"";
                    }
        ...
        {
            if (this.ys_strSpecialParams != null)
            {
                string str = "";
                string[] strArray = this.ys_strSpecialParams.Split(new char[] { '$' });
                for (int i = 0; i < strArray.Length; i++)
                {
                    if (i > 0)
                    {
        ...
        protected void makeHtmlFile(Template tempRe, bool existFlag, string classID, string NewsID)
        {
            ...
            if (tempRe.MyTempType == TempType.News)
            {
                ...
                int index = finallyContent.IndexOf("<!-YS:STAR=");
                int num2 = finallyContent.IndexOf("YS:END->");
                if (index > -1)
                {
                    ...
                    Regex regex = new Regex(@"(\[YS:Page=(?<p>[\s\S]+?)\$\])|(\[YS:Page\])", RegexOptions.IgnoreCase);
                    string[] strArray = Regex.Split(regex.Replace(input, "[YS:Page]"), @"\[YS:Page\]", RegexOptions.IgnoreCase);
                    int length = strArray.Length;
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        General.WriteHtml(General.ReplaceResultPage(NewsID, str8.Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), fileName, eXName, length, i + 1, 0), filePath);
                    }
                    if (length > 0)
                    {
                        if (!existFlag)
            ...
            General.WriteHtml(tempRe.FinallyContent.Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), this.SiteRootPath + this.saveNewsPath);
            if (!existFlag)
            {
                this.templateList.Add(tempRe);
            }
        ...
        public static bool PublishSingleJsFile(int tid)
        {
            try
            {
                YSCMS.CMS.NewsJS sjs = new YSCMS.CMS.NewsJS();
                YSCMS.Model.NewsJS model = sjs.GetModel(tid);
                string str = "";
                    ...
                    if (template.MyTempType == TempType.News)
                    {
                        ...
                        if (index > -1)
                        {
                            ...
                            for (int i = 0; i < length; i++)
                            {
                                ...
                                General.WriteHtml(General.ReplaceResultPage(row["NewsID"].ToString(), str12.Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), fileName, eXName, length, i + 1, 0), filePath);
                            }
                        }
                        else
                        {
                            General.WriteHtml(finallyContent.Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), str3 + str);
                        }
                    }
        ...
        protected void ReplaceTemp(Template tempRe, bool existFlag, string savePath, string id, string ContentType)
        {
            ...
            if ((((tempRe.MyTempType == TempType.Class) || (tempRe.MyTempType == TempType.Special)) || (tempRe.MyTempType == TempType.ChClass)) || (tempRe.MyTempType == TempType.Chspecial))
            {
                ...
                string currentSytle = "";
                string otherStyle = "";
                string pattern = @"\{YS\:PageLinksStyle=\w+\|\w+\}";
                ...
                int length = finallyContent.IndexOf("{Yuesi:NewsLIST}");
                int num3 = finallyContent.IndexOf("{/Yuesi:NewsLIST}");
                ...
                if ((num3 > length) && (length > -1))
                {
                    ...
                    string str13 = @"\{\$YS\:P[01]\}\{Page\:\d\$[^\$]{0,6}\$[^\$]{0,20}\}";
            ...
            General.WriteHtml(tempRe.FinallyContent.Replace("{Yuesi:NewsLIST}", "").Replace("{/Yuesi:NewsLIST}", "").Replace("{PageLists}", "").Replace("{$YS:P1}", newValue), savePath);
            if (!existFlag)
            {
                this.templateList.Add(tempRe);
            }
        ...
        {
            string classid = "";
            if (this.ys_strClassIsPageParam != null)
            {
                int num;
                string[] strArray = this.ys_strClassIsPageParam.Split(new char[] { '$' });
        ...
        public bool isClassIndex
        {
            get
            {
                return this.ys_isClassIndex;
            }
            set
            {
                this.ys_isClassIndex = value;
            }
        }
        ...
        public bool IsPubClass
        {
            get
            {
                return this.ys_isPubClass;
            }
            set
            {
                this.ys_isPubClass = value;
            }
        }
        ...
        public bool IsPubIsPage
        {
            get
            {
                return this.ys_isPubIsPage;
            }
            set
            {
                this.ys_isPubIsPage = value;
            }
        }
        ...
        public bool IsPublishIndex
        {
            get
            {
                return this.ys_isPubindex;
            }
            set
            {
                this.ys_isPubindex = value;
            }
        }
        ...
        public bool IsPubNews
        {
            get
            {
                return this.ys_isPubNews;
            }
            set
            {
                this.ys_isPubNews = value;
            }
        }
        ...
        public bool IsPubSpecial
        {
            get
            {
                return this.ys_isPubSpecial;
            }
            set
            {
                this.ys_isPubSpecial = value;
            }
        }
        ...
        public string StrClassIsPageParam
        {
            set
            {
                this.ys_strClassIsPageParam = value;
            }
        }
        ...
        public string strSpecialParams
        {
            set
            {
                this.ys_strSpecialParams = value;
            }
        }
