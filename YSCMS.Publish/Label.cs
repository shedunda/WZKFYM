namespace YSCMS.Publish
{
    ...
    public class Label
    {
        ...
        public static Label GetLabel(string labelname)
        {
            ...
            if (Regex.Match(input, @"\{YS_FREE_[^\}]+\}", RegexOptions.Compiled).Success)
            {
                return new FreeLabel(labelname, LabelType.Free);
            }
            if (Regex.Match(input, @"\{YS_DYN[^\}]+\}", RegexOptions.Compiled).Success)
            {
                return new DynamicLabel(labelname, LabelType.Class);
            }
            if (Regex.Match(input, @"\{YS_CH\$\d+_[^\}]+\}", RegexOptions.Compiled).Success)
            {
                string str2 = input.Replace("{YS_CH$", "");
                int index = str2.IndexOf("_");
                string s = str2.Substring(0, index);
                return new ChannelLabel(labelname, LabelType.Channel) { CHID = int.Parse(s) };
            }
            if (!Regex.Match(input, @"\{YS_[^\}]+\}", RegexOptions.Compiled).Success)
            {
