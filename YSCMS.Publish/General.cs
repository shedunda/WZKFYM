namespace YSCMS.Publish
{
    ...
    public class General
    {
        ...
        public static string GetPageLinkTextStr(string _PageStyles, int _CurrentPage, int _PageCount, int _PageLinkCount, string _fileName, string _EXName)
        {
            ...
            string str = "";
            string str2 = " class=\"yuesi_pagebox\"";
            string str3 = " class=\"yuesi_pagebox_num_nonce\"";
            string str4 = " class=\"yuesi_pagebox_num\"";
                    ...
                    if (cHNewsSavePath["isDelPoint"].ToString() == "0")
                    {
                        ...
                        if (template.MyTempType == TempType.ChNews)
                        {
                            ...
                            if (index > -1)
                            {
                                ...
                                string[] strArray = finallyContent.Substring(index + 11, (num2 - index) - 11).Split(new string[] { "[YS:PAGE]" }, StringSplitOptions.RemoveEmptyEntries);
                                ...
                                for (int i = 0; i < length; i++)
                                {
                                    ...
                                    WriteHtml(ReplaceResultPage(cHNewsSavePath["id"].ToString(), str13.Replace("[YS:PAGE]", "").Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), fileName, eXName, length, i + 1, 0), filePath);
                                }
                            }
                            else
                            {
                                WriteHtml(finallyContent.Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), str3 + str);
                            }
                        }
                        flag = true;
                    }
        ...
        public static bool PublishClassIndex(string ClassID)
        {
            ...
            try
            {
                ...
                if (classById != null)
                {
                    ...
                    WriteHtml(template.FinallyContent.Replace("{Yuesi:NewsLIST}", "").Replace("{/Yuesi:NewsLIST}", "").Replace("{$YS:P1}", newValue), filePath);
                }
                return true;
            }
            catch (Exception exception)
            ...
            try
            {
                ...
                while (singlePageClass.Read())
                {
                    ...
                    else
                    {
                        ...
                        input = new Regex(@"(\[YS:PAGE=(?<p>[\s\S]+?)\$\])|(\[YS:PAGE\])", RegexOptions.IgnoreCase).Replace(input, "[YS:PAGE]");
                        string[] strArray = Regex.Split(input, @"\[YS:PAGE\]", RegexOptions.IgnoreCase);
                        int pageCount = 0;
                        if (strArray != null)
                        {
                        ...
                        if (template.MyTempType == TempType.News)
                        {
                            ...
                            if (index > -1)
                            {
                                ...
                                for (int i = 0; i < length; i++)
                                {
                                    ...
                                    WriteHtml(ReplaceResultPage(newsSavePath["NewsID"].ToString(), str12.Replace("YS:END->", "").Replace("<!-YS:STAR=", ""), fileName, eXName, length, i + 1, 0), filePath);
                                }
                            }
                            else
                            {
        ...
        public static bool PublishXML(string ClassID)
        {
            ...
            try
            {
                ...
                Regex regex2 = new Regex(@"(\[YS:PAGE=(?<p>[\s\S]+?)\$\])|(\[YS:PAGE\])", RegexOptions.IgnoreCase);
        ...
        public void ReplaceTempg(Template tempRe, string savePath, string id, string ContentType)
        {
            ...
            WriteHtml(tempRe.FinallyContent.Replace("{Yuesi:NewsLIST}", "").Replace("{/Yuesi:NewsLIST}", "").Replace("{PageLists}", "").Replace("{$YS:P1}", newValue), savePath);
        }
        ...
        public static void WriteHtml(string Content, string FilePath)
        {
            ...
            //if (Public.readparamConfig("Open", "Yuesi") == "11")
            //{
            //    str2 = str2 + "<script src='http://pw.cnzz.com/c.php?id=" + Public.readparamConfig("SiteID", "Yuesi") + "'  type=\"text/javascript\" charset='gb2312'></script>\r\n";
            //}
            ...
            string str4 = string.Concat(new object[] { str3, "\r\n<!--Created by (c) 1993-2014 Yuesi Information Technology Co., Ltd. ", verConfig.Productversion, " Published at ", DateTime.Now, "-->\r\n" });
            try
            {
                FtpConfig config;
