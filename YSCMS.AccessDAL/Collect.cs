...
namespace YSCMS.AccessDAL
{
    public class Collect : DbBase, ICollect
    {
        ...
        private OleDbConnection conyuesi;
        ...
        public void StoreNews(bool UnStore, int[] id, out int nSucceed, out int nFailed)
        {
            ...
            conyuesi = new OleDbConnection(Database.YuesiConnectionString);
        ...
        private void StoreStep(string Sql)
        {
            int IsAudit = 3;
            int AID = 0;
            object obj = DbHelper.ExecuteScalar(conyuesi, CommandType.Text, "select max(id) from " + Pre + "news", null);
            ...
            foreach (DataRow r in dt.Rows)
            {
                try
                {
                    ...
                    IDataReader rd = DbHelper.ExecuteReader(conyuesi, CommandType.Text, Sql, null);
                    if (rd.Read())
                    {
                        #region 赋值
                    ...
                    if (isExistedClassID)
                    {
                        ...
                        while (Convert.ToInt32(DbHelper.ExecuteScalar(conyuesi, CommandType.Text, "select count(ID) from " + Pre + "news where NewsID='" + NewsID + "' or FileName='" + FileName + "'", null)) > 0)
                        {
                        ...
                        param[13].Value = YSCMS.Global.Current.UserName;
                        ...
                        //'" + FileName + "','.html',0,0,0,0,1,1,0,1,0,'YS_News',0,0,0)";

                        DbHelper.ExecuteNonQuery(conyuesi, CommandType.Text, Sql, param);
        ...
        private void StoreEnd()
        {
            if (connetion != null && connetion.State == ConnectionState.Open)
                connetion.Close();
            if (conyuesi != null && conyuesi.State == ConnectionState.Open)
                conyuesi.Close();
        }
