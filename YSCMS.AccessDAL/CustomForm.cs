...
namespace YSCMS.AccessDAL
{
    public class CustomForm : DbBase, ICustomForm
    {
        void ICustomForm.Edit(CustomFormInfo info)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Sql = "insert into ys_tables(tabname) values('" + tab + "')";
                    DbHelper.ExecuteNonQuery(tran, CommandType.Text, Sql, null);
                    #endregion
                }
                tran.Commit();
		...
		StringBuilder ICustomForm.GetFromData()
		{
			...
			try
			{
				string Sql = "SELECT id,formname,formtablename FROM ys_customform";
				dt = DbHelper.ExecuteTable(cn, CommandType.Text, Sql, null);
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					if (i == 0)
					{
						DataTable dts = DbHelper.ExecuteTable(cn, CommandType.Text, "SELECT * FROM ys_customform_item WHERE formid=" + dt.Rows[i]["id"], null);
						if (dts == null)
						{
