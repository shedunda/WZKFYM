...
namespace YSCMS.AccessDAL
{
    public class RootPublic : DbBase, IRootPublic
    {
        ...
        {
            string USQL = "Select UserGroupNumber From " + Pre + "sys_user Where UserNum=@UserNum and SiteID='" + YSCMS.Global.Current.SiteID + "'";
            OleDbParameter Param = new OleDbParameter("@UserNum", userNum);
        ...
        public string GetIDGroupNumber(string groupNumber)
        {
            ...
            string Sql = "Select id From " + Pre + "user_Group Where GroupNumber=@GroupNumber and SiteID='" + YSCMS.Global.Current.SiteID + "'";
        ...
        {
            string gflg = "(c) 1993-2014 Yuesi Information Technology Co., Ltd." + verConfig.Productversion;
        ...
        {
            string gflg = "service@yuesisoft.com";
        ...
        public string SaveClassFilePath(string siteId)
        {
            ...
            else
            {
                ...
                {
                    gflg = rdrs.Rows[0]["SaveDirPath"].ToString() + "/" + rdrs.Rows[0]["EName"].ToString() + "/" + YSCMS.Config.UIConfig.dirHtml;
        ...
        {
            string Sql = "select id,GroupNumber,GroupName from " + Pre + "user_Group where SiteID='" + YSCMS.Global.Current.SiteID + "'";
        ...
        {
            string Sql = "Select id,ChannelID,ParentID,CName from " + Pre + "news_site where isRecyle=0 and isLock=0 and IsURL =0 and ParentID='" + YSCMS.Global.Current.SiteID + "' order by id desc";
            DataTable rdr = DbHelper.ExecuteTable(CommandType.Text, Sql, null);
            return rdr;
        }
        ...
        {
            string Sql = "Select id,ClassID,Sname,(Select Count(id) from " + Pre + "sys_LabelStyle where a.ClassId=ClassID and isRecyle=0 and siteID='" + YSCMS.Global.Current.SiteID + "') as HasSub  from " + Pre + "sys_styleclass a where isRecyle=0 and siteID='" + YSCMS.Global.Current.SiteID + "' order by id desc";
            DataTable rdr = DbHelper.ExecuteTable(CommandType.Text, Sql, null);
            return rdr;
        }
        ...
        public DataTable GetselectLabelList1(string classID)
        {
            ...
            string Sql = "select ID,styleID,ClassID,StyleName,Description,Content from " + Pre + "sys_LabelStyle where ClassId=@ClassID and siteID='" + YSCMS.Global.Current.SiteID + "' order by id desc";
            DataTable rdr = DbHelper.ExecuteTable(CommandType.Text, Sql, param);
            return rdr;
        }
        ...
        public IDataReader GetAjaxsNewsList(string parentId)
        {
            ...
            string Sql = "Select ClassID,ClassCName,(Select Count(id) from " + Pre + "news_Class where ParentID=a.ClassID and isRecyle=0 and isUrl=0 and isPage=0 and islock=0) as HasSub from " + Pre + "news_Class a where ParentID=@ParentId and isRecyle=0 and isUrl=0 and SiteID='" + YSCMS.Global.Current.SiteID + "' and isPage=0 and islock=0 order by OrderID desc,id desc";
        ...
        {
            OleDbParameter param = new OleDbParameter("@ParentId", parentId);
            string Sql = "Select SpecialID,SpecialCName,(Select Count(id) from " + Pre + "news_special where ParentID=a.SpecialID and isRecyle=0) as HasSub from " + Pre + "news_special a where ParentID=@ParentId and isRecyle=0 and SiteID='" + YSCMS.Global.Current.SiteID + "' order by id desc";
            return DbHelper.ExecuteReader(CommandType.Text, Sql, param);
        }
        ...
        {
            OleDbParameter param = new OleDbParameter("@ParentID", parentID);
            string Sql = "Select ClassID,ClassCName,ParentID from " + Pre + "news_class where isURL=0 and isLock=0 and isRecyle=0 and isPage<>1 and ParentID=@ParentID and SiteID ='" + YSCMS.Global.Current.SiteID + "' order by OrderID desc,Id desc";
            DataTable rdr = DbHelper.ExecuteTable(CommandType.Text, Sql, param);
            return rdr;
        }
        ...
        {
            OleDbParameter param = new OleDbParameter("@ParentID", parentID);
            string Sql = "Select SpecialID,SpecialCName,ParentID from " + Pre + "news_special where isLock=0 and isRecyle=0 and ParentID=@ParentID and SiteID ='" + YSCMS.Global.Current.SiteID + "' order by Id desc";
            DataTable rdr = DbHelper.ExecuteTable(CommandType.Text, Sql, param);
            return rdr;
        }
        ...
        public string GetClassEName(string classID)
        {
            string _STR = "";
            OleDbParameter param = new OleDbParameter("@ClassID", classID);
            string Sql = "Select ClassEName From " + Pre + "news_class where ClassID=@ClassID and SiteID ='" + YSCMS.Global.Current.SiteID + "'";
            DataTable dt = DbHelper.ExecuteTable(CommandType.Text, Sql, param);
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
        ...
        public void DelUserAllInfo(string userNum)
        {
            //删除投稿
            OleDbParameter param = new OleDbParameter("@UserNum", userNum);
            string consql = "delete from " + Pre + "user_Constr where UserNum = @UserNum and SiteID='" + YSCMS.Global.Current.SiteID + "'";
