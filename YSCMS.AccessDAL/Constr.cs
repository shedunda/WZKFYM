...
namespace YSCMS.AccessDAL
{
	public class Constr : DbBase, IConstr
	{
		...
		public DataTable GetPage1(int PageIndex, int PageSize, out int RecordCount, out int PageCount, params SQLConditionInfo[] SqlCondition)
		{
            ...
            string Condition = "(select DISTINCT UserNum from " + Pre + "user_Constr where Mid(Contrflg,3,1) = '1' and SiteID='" + YSCMS.Global.Current.SiteID + "') UserNum1";
		...
		public int Add6(string PCId, string ConstrPayName, string gpoint, string ipoint, int moneys1, string Gunit)
		{
            ...
            param[5].Value = Gunit;
            string Sql = "insert into " + Pre + "sys_ParmConstr(PCId,ConstrPayName,gPoint,iPoint,[money],Gunit,SiteID) values(@PCId,@ConstrPayName,@gpoint,@ipoint,@money,@Gunit,'" + YSCMS.Global.Current.SiteID + "')";
