...
namespace YSCMS.AccessDAL
{
    public class Pagination : DbBase, IPagination
    {
        ...
        //protected string[] user_friend_friendmanage ={ "id", "id,HailFellow,FriendName,CreatTime,(select count(*) from " + YSCMS.Config.UIConfig.dataRe + "User_friend where busernum=" + YSCMS.Config.UIConfig.dataRe + "User_FriendClass.usernum) as CNT", "User_FriendClass where UserNum=@UserNum or gdfz=1", "order by Id desc" };
        ...
        //投稿后台
        protected string[] manage_Contribution_Constrchicklist = { "id", "id,ConID,Title,creatTime,UserNum,Source,Tags,Author,ispass,Contrflg", "User_Constr where Mid(Contrflg,3,1) = '1' and isadmidel=0 and isCheck=1 and ispass=0 and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by Id desc" };

        protected string[] manage_Contribution_ConstrList = { "id", "id,ConID,Title,creatTime,Source,Tags,Contrflg,Author,ispass", "User_Constr where Mid(Contrflg,3,1) = '1' and isadmidel=0 and isCheck=0 and ispass=0 and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by Id desc" };
        ...
        protected string[] manage_user_discussclass_3 = { "id", "id,DcID,Cname,Content", "User_DiscussClass where indexnumber='0' and Cname like cstr(titlem) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by Id desc" };
        ...
        protected string[] manage_user_discuss_2 = { "id", "id,DisID,Cname,UserName,Creatime,Authoritymoney", "user_Discuss where Cname like cstr(titlem) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by Id desc" };
        ...
        protected string[] manage_survey_setClass_2 = { "vid", "vid,ClassName,Description", "vote_Class where VID like cstr(@VID) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by vid desc" };

        protected string[] manage_survey_setClass_3 = { "vid", "vid,ClassName,Description", "vote_Class where ClassName like cstr(@ClassName) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by vid desc" };

        protected string[] manage_survey_setClass_4 = { "vid", "vid,ClassName,Description", "vote_Class where Description like cstr(@Description) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by vid desc" };
        //调查选项
        protected string[] manage_survey_setItem_1 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where TID like cstr(@TID) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_2 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_3 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where ItemName like cstr(@ItemName) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_4 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where ItemValue like cstr(@ItemValue) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_5 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where PicSrc like cstr(@PicSrc) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_6 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where DisColor like cstr(@DisColor) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_7 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where VoteCount like cstr(@VoteCount) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setItem_8 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where ItemDetail like cstr(@ItemDetail) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };
        //多步投票管理
        ...
        protected string[] manage_survey_setSteps_2 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where SID like cstr(@SID) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by sid desc" };

        protected string[] manage_survey_setSteps_3 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where TIDS like cstr(@TIDS) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by sid desc" };

        protected string[] manage_survey_setSteps_4 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where Steps like cstr(@Steps) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by sid desc" };

        protected string[] manage_survey_setSteps_5 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where TIDU like cstr(@TIDU) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by sid desc" };
        //投票主题
        ...
        protected string[] manage_survey_setTitle_2 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where Title like cstr(@Title) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setTitle_3 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where VID like cstr(@VID) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setTitle_4 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where MaxNum like cstr(@MaxNum) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setTitle_5 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where StartDate like cstr(@StartDate) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setTitle_6 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where EndDate like cstr(@EndDate) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };

        protected string[] manage_survey_setTitle_7 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where ItemMode like cstr(@ItemMode) and SiteID='" + YSCMS.Global.Current.SiteID + "'", "order by TID desc" };
        //自定义字段
