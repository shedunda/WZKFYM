...
namespace YSCMS.AccessDAL
{
    ...
    public partial class News : DbBase, INews
    {
        ...
        /// 增加一条数据
        /// </summary>
        public void Add(YSCMS.Model.News model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(YSCMS.Model.News model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public void Copy_news(string ClassID, string sOrgNews, string NewsID, string FileName)
        {
            ...
            YSCMS.AccessDAL.NewsClass sNewclass = new NewsClass();
        ...
        /// 得到一个对象实体
        /// </summary>
        public YSCMS.Model.News GetModel(string NewsID)
        {
            ...
            YSCMS.Model.News model = new YSCMS.Model.News();
        ...
        public DataTable GetPage(string SpecialID, string Editor, string ClassID, string sKeywrd, string DdlKwdType, string sChooses, string SiteID, int PageIndex, int PageSize, out int RecordCount, out int PageCount)
        {
            ...
            switch (sChoose)
            {
                ...
                case "Slide":
                    ...
                    break;
                case "my":
                    sFilter += " and Editor='" + YSCMS.Global.Current.UserName + "'";
                    break;
                case "isHtml":
            ...
            {
                sFilter += " and a.SiteID='" + YSCMS.Global.Current.SiteID + "'";
            }
        ...
        {
            string Sql = "delete from " + Pre + "News_Sub where NewsID='" + UnID + "' and SiteID='" + YSCMS.Global.Current.SiteID + "'";
            DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
        }
        /// <summary>
        /// 插入子新闻
        ...
        /// <param name="uc"></param>
        /// <returns></returns>
        private OleDbParameter[] insertNewsContentParameters(YSCMS.Model.News uc)
        {
        ...
        public void UpdateNewsContent(YSCMS.Model.News uc)
        {
        ...
        /// </summary>
        /// <param name="uc2"></param>
        public void InsertNewsContent(YSCMS.Model.News uc)
        {
            OleDbParameter[] parm = insertNewsContentParameters(uc);
        ...
        public void SetDefineSign(string DsNewsID, string DsEName, string DsNewsTable, int DsType, string DsContent, string DsApiID)
        {
            ...
            {
                string Sql = "insert into " + Pre + "define_save (DsNewsID,DsEname,DsNewsTable,DsType,DsContent,DsApiID,SiteID) VALUES ('" + DsNewsID + "','" + DsEName + "','" + DsNewsTable + "'," + DsType + ",'" + DsContent + "','" + DsApiID + "','" + YSCMS.Global.Current.SiteID + "')";
                DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
            }
        }
        #endregion
        ...
        /// <param name="uc"></param>
        /// <returns></returns>
        private OleDbParameter[] intsertTTParameters(YSCMS.Model.NewsContentTT uc)
        {
        ...
        /// </summary>
        /// <param name="uc2"></param>
        public void IntsertTT(YSCMS.Model.NewsContentTT uc)
        {
        ...
        {
            string SQLTF = "select id from " + Pre + "News_Gen where Cname='" + _TempStr.Trim() + "' and gType=" + _num + " and SiteID='" + YSCMS.Global.Current.SiteID + "'";
            ...
            if (rdr != null)
            {
                if (rdr.Rows.Count == 0)
                {
                    ...
                    Sql += ") values (";
                    Sql += "'" + _TempStr + "'," + _num + ",'" + _URL + "','" + _EmailURL + "',0,'" + YSCMS.Global.Current.SiteID + "')";
                    DbHelper.ExecuteNonQuery(CommandType.Text, Sql, null);
                }
                rdr.Clear(); rdr.Dispose();
            }
        ...
        {
            string Sql = "select id,NewsID,DataLib,NewsType from " + Pre + "news where islock=0 and isRecyle=0 and siteID='" + YSCMS.Global.Current.SiteID + "' order by CreatTime desc,id desc";
            return DbHelper.ExecuteTable(CommandType.Text, Sql, null);
        }
        ...
        public string GetNewsReview(string ID, string gType)
        {
            ...
            string dim = YSCMS.Config.UIConfig.dirDumm.Trim();
                ...
                if (gType == "class")
                {
                    ...
                    while (dt.Read())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                newspath1 = dim + "/list-" + dt["ClassID"].ToString() + YSCMS.Config.UIConfig.extensions;
                            }
                            else
                            {
                                if (ReadType == "1")
                                {
                                    ...
                                    {
                                        newspath1 = dim + "/page-" + dt["ClassID"].ToString() + YSCMS.Config.UIConfig.extensions;
                                    }
                                    else
                                    {
                                        newspath1 = dim + "/list-" + dt["ClassID"].ToString() + YSCMS.Config.UIConfig.extensions;
                ...
                else
                {
                    ...
                    while (dt.Read())
                    {
                        if (dt["NewsType"].ToString() != "2")
                        {
                            if (dt["isDelPoint"].ToString() != "0")
                            {
                                newspath1 = dim + "/content-" + dt["NewsID"].ToString() + YSCMS.Config.UIConfig.extensions;
                            }
                            else
                            {
                                if (ReadType == "1")
            ...
            else
            {
                ...
                while (dt.Read())
                {
                    if (dt["isDelPoint"].ToString() != "0")
                    {
                        newspath1 = dim + "/special-" + dt["SpecialID"].ToString() + YSCMS.Config.UIConfig.extensions;
                    }
                    else
                    {
                        if (ReadType == "1")
        ...
        public int AddComment(YSCMS.Model.Comment ci)
        {
