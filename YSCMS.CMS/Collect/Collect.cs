...
namespace YSCMS.CMS.Collect
{
	/// <summary>
	/// 采集类
	/// </summary>
	public class Collect {
		private YSCMS.IDAL.ICollect dal;
		private string ErrorMsg = "";
		/// <summary>
		/// 构造函数
		/// </summary>
		public Collect() {
			dal = YSCMS.DALFactory.DataAccess.CreateCollect();
		}
		#region 采集入库
		/// <summary>
		/// 是否保存远程图片
		...
		public void Collecting(int folderid, int num, bool bnorepeat) {
			...
			if (bool.Parse(r["SaveRemotePic"].ToString())) {
				#region 远程图片
				string rtpath = YSCMS.Config.UIConfig.dirFile;
		...
		private int CollectPage(string Url, DataRow r, bool norepeat) {
			...
			if (pn.Content != null && !pn.Content.Trim().Equals("") && !pn.Title.Trim().Equals("")) {
				YSCMS.Model.CollectNewsInfo ninf = new YSCMS.Model.CollectNewsInfo();
				...
				if (YSCMS.Config.UIConfig.enableAutoPage == "true") {
					ninf.Content = Common.Input.AutoSplitPage(ninf.Content, int.Parse(YSCMS.Config.UIConfig.splitPageCount));
				}
				//获取内容中新闻图片的数量
