...
namespace YSCMS.CMS.Collect {
	...
	public class RemoteResource {
		...
		/// <param name="LocalURLDirectory">要将文件保存到本地服务器的虚拟目录，用于替换原来的远程链接地址，如：http://www.yuesi.com/remoteres,可以为空,也可以为../一个或多个。</param>
		/// <param name="LocalPhysicalDirectory">要将文件保存到本地服务器的磁盘路径，如：C:\Inetpub\wwwroot\remoteres,如果不存在可以创建</param>
		/// <param name="RemoteUrl">用于处理相对路径（如src="../images/yuesi.gif"）的资源，如果为空，则只取完整路径的资源，以http(或https,ftp,rtsp,mms)://开头</param>
