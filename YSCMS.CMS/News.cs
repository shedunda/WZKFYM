...
namespace YSCMS.CMS
{
    ...
    public partial class News
    {
        ...
        /// 得到一个对象实体
        /// </summary>
        public YSCMS.Model.News GetModel(string Id)
        {
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public YSCMS.Model.News GetModelByCache(string Id)
        {
            ...
            return (YSCMS.Model.News)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSCMS.Model.News> GetModelList(string strWhere)
        {
            DataTable ds = dal.GetList(strWhere);
            return DataTableToList(ds);
        }
        public List<YSCMS.Model.News> GetPageByClass(string where, int PageIndex, int PageSize, out int RecordCount, out int PageCount)
        {
            DataTable ds = dal.GetPageByClass(where,PageIndex,PageSize,out RecordCount,out PageCount);
            return DataTableToList(ds);
        }
        public List<YSCMS.Model.News> GetModelList(int top, string strWhere, string filedOrder)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<YSCMS.Model.News> DataTableToList(DataTable dt)
        {
            List<YSCMS.Model.News> modelList = new List<YSCMS.Model.News>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                YSCMS.Model.News model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new YSCMS.Model.News();
        ...
        public bool DeleteNewsHtmlFile(string NewsID)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (_NewsType != "2")
                {
                    ...
                    string _DeletePath = "/" + YSCMS.Config.UIConfig.dirDumm + "/" + _ClassSavePath + "/" + _SaveClassframe + "/" + _NewsSavePath + "/" + _FileName + _FileEXName;
                    ...
                    while (_FileIsExisted)
                    {
                        ...
                        i++;
                        _DeletePath = "/" + YSCMS.Config.UIConfig.dirDumm + "/" + _ClassSavePath + "/" + _SaveClassframe + "/" + _NewsSavePath + "/" + _FileName + "_" + i.ToString() + _FileEXName;
                        _DeletePath = HttpContext.Current.Server.MapPath(_DeletePath.Replace("//", "/").Replace("//", "/"));
