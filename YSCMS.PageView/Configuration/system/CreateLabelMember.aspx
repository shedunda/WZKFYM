<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CreateLabelMember.aspx.cs" Inherits="YSCMS.PageView.Configuration.system.CreateLabelMember" %>
...
<div id="dialog-message" title="提示"></div>
    ...
    <div class="newxiu_base">
     ...
     <tr style="display:none;" id="aTrStyleID">
            ...
            <td width="79%" align="left">
                <asp:TextBox ID="aStyleID" runat="server"></asp:TextBox> <img src="/CSS/imges/bgxiu_14.gif" align="middle" style="cursor:pointer;" alt="选择样式" onclick="javascript:selectFile('aStyleID','样式','style','500','300');ListLabel.aStyleID.focus();"  /> 
           </td>
          </tr>
          <tr style="display:none;" id="TrLoginCSS">
            <td align="right" style="width: 28%">登陆框样式：</td>
            <td width="79%" align="left">
                <asp:TextBox ID="LoginCSS" runat="server"></asp:TextBox> <img src="/CSS/imges/bgxiu_14.gif" align="middle" style="cursor:pointer;" title="选择图片"  onclick="javascript:selectFile('LoginCSS','图片','pic','500','300');ListLabel.LoginCSS.focus();" /> 可以填写CSS或图片地址。填写图片地址直接输入地址。
           </td>
          </tr>
          ...
          <tr style="display:none;" id="TrRegCSS">
            ...
            <td width="79%" align="left">
                <asp:TextBox ID="RegCSS" runat="server"></asp:TextBox> <img src="/CSS/imges/bgxiu_14.gif" align="middle" style="cursor:pointer;" title="选择图片"  onclick="javascript:selectFile('RegCSS','图片','pic','500','300');ListLabel.RegCSS.focus();" /> 可以填写CSS或图片地址。填写图片地址直接输入地址。
           </td>
          </tr>
          ...
          <tr style="display:none;" id="TrPassCSS">
            ...
            <td width="79%" align="left">
                <asp:TextBox ID="PassCSS" runat="server"></asp:TextBox> <img src="/CSS/imges/bgxiu_14.gif" align="middle" style="cursor:pointer;" title="选择图片"  onclick="javascript:selectFile('PassCSS','图片','pic','500','300');ListLabel.PassCSS.focus();" /> 可以填写CSS或图片地址。填写图片地址直接输入地址。
           </td>
          </tr>
          ...
          <tr id="TrNaviPic" style="display:none;">
            ...
            <td width="79%" align="left"><asp:TextBox ID="NaviPic" runat="server" class="input9" ReadOnly="true"></asp:TextBox>
              &nbsp;
              <input class="form" type="button" value="选择图片"  onclick="javascript: selectFile('NaviPic', '图片', 'pic', '500', '300'); ListLabel.NaviPic.focus();" /></td>
          </tr>          
          <tr id="TrClassId" style="display:none;">
            ...
            <td width="79%" align="left"><asp:TextBox ID="ClassName" runat="server" class="input9" ReadOnly="true"></asp:TextBox>
              ...
              <input class="form" type="button" value="选择栏目"  onclick="javascript: selectFile('ClassName,ClassId', '栏目列表', 'newsclass', '500', '300'); ListLabel.ClassId.focus();" /></td>
          </tr> 
          ...
          <tr id="TrStyleID" style="display:none;">
            ...
            <td width="79%" align="left"><asp:TextBox ID="StyleID" runat="server" class="input9" ReadOnly="true"></asp:TextBox>
              &nbsp;
              <input class="form" type="button" value="选择样式"  onclick="javascript: selectFile('StyleID', '样式', 'style', '500', '300'); ListLabel.StyleID.focus();" /><span id="sapnStyleID"></span></td>
          </tr>
...
<script language="javascript" type="text/javascript">
    ...
    function ReturnDivValue() {
        ...
        switch (ListLabel.LabelType.value) {
            case "UserLogin":
                if (ListLabel.LoginP.value == "" && ListLabel.aStyleID.value == "") {
                    document.getElementById("spanLoginP").innerHTML = "<span style='color:red'>显示方式或引用样式任填一样</span>";
                    CheckStr = false;
                }
                rvalue = "[YS:unLoop,YS:SiteID=<%Response.Write(APIID); %>,YS:LabelType=UserLogin,YS:LoginP=" + ListLabel.LoginP.value;
                if (ListLabel.FormCSS.value != "") { rvalue += ",YS:FormCSS=" + ListLabel.FormCSS.value; }
                if (ListLabel.LoginCSS.value != "") { rvalue += ",YS:LoginCSS=" + ListLabel.LoginCSS.value; }
                if (ListLabel.RegCSS.value != "") { rvalue += ",YS:RegCSS=" + ListLabel.RegCSS.value; }
                if (ListLabel.PassCSS.value != "") { rvalue += ",YS:PassCSS=" + ListLabel.PassCSS.value; }
                if (ListLabel.aStyleID.value != "") { rvalue += ",YS:StyleID=" + ListLabel.aStyleID.value; }
                rvalue += "][/YS:unLoop]";
                if (CheckStr)
            ...
            case "TopUser":
                if (checkIsNumber(ListLabel.Number, document.getElementById("spanNumber"), "循环数目只能为正整数"))
                    CheckStr = false;
                rvalue = "[YS:Loop,YS:SiteID=<%Response.Write(APIID); %>,YS:LabelType=" + ListLabel.LabelType.value;

                if (ListLabel.Number.value != "") { rvalue += ",YS:Number=" + ListLabel.Number.value; }
                if (ListLabel.TopUserType.value != "") { rvalue += ",YS:TopUserType=" + ListLabel.TopUserType.value; }
                if (ListLabel.ShowNavi.value != "") { rvalue += ",YS:ShowNavi=" + ListLabel.ShowNavi.value; }
                if (ListLabel.ShowNavi.value != "" && ListLabel.ShowNavi.value != "4") { rvalue += ",YS:NaviCSS=" + ListLabel.NaviCSS.value; }
                if (ListLabel.ShowNavi.value == "4")
                { if (ListLabel.NaviPic.value != "") { rvalue += ",YS:NaviPic=" + ListLabel.NaviPic.value; } }
                if (ListLabel.CSS.value != "") { rvalue += ",YS:CSS=" + ListLabel.CSS.value; }
                if (ListLabel.PointParam.value != "") { rvalue += ",YS:PointParam=" + ListLabel.PointParam.value; }
                rvalue += "][/YS:Loop]";
                if (CheckStr)
                    parent.parent.getValue(rvalue);
            ...
            case "GroupMember":
                ...
                { if (ListLabel.NaviPic.value != "") { rvalue += ",YS:NaviPic=" + ListLabel.NaviPic.value; } }
                if (ListLabel.Cols.value != "") { rvalue += ",YS:Cols=" + ListLabel.Cols.value; }
                if (ListLabel.GroupType.value != "") { rvalue += ",YS:GroupType=" + ListLabel.GroupType.value; }
                if (ListLabel.isDiv.value != "") { rvalue += ",YS:isDiv=" + ListLabel.isDiv.value; }
                if (ListLabel.CSS.value != "") { rvalue += ",YS:CSS=" + ListLabel.CSS.value; }
                if (ListLabel.isDiv.value == "true") {
                    if (ListLabel.ulID.value != "") { rvalue += ",YS:ulID=" + ListLabel.ulID.value; }
                ...
                if (ListLabel.TitleNumer.value != "") { rvalue += ",YS:TitleNumer=" + ListLabel.TitleNumer.value; }
                if (ListLabel.ShowM.value != "") { rvalue += ",YS:ShowM=" + ListLabel.ShowM.value; }
                rvalue += "][/YS:Loop]";
                if (CheckStr)
                    parent.parent.getValue(rvalue);
                break;
