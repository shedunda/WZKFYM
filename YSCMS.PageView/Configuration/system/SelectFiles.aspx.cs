...
{
    public partial class SelectFiles : YSCMS.PageBasic.DialogPage
    {
        public SelectFiles()
        {
            BrowserAuthor = EnumDialogAuthority.ForAdmin;
        }
        private string str_dirMana = YSCMS.Config.UIConfig.dirDumm;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (_FileType == "pic")
            {
                str_dirFile = YSCMS.Config.UIConfig.dirFile;
            ...
            {
                str_dirFile = YSCMS.Config.UIConfig.dirTemplet;
                tf = false;
            }
            ...
            {
                string _sitePath = str_dirMana + "\\" + YSCMS.Config.UIConfig.dirSite + "\\" + YSCMS.Global.Current.SiteID + "\\" + str_dirFile;
        ...
        protected string GetDirFile(string dir, string ParPath)
        {
            ...
            else
            {
                ...
                {
                    _str_dirFileTF = str_dirMana + "\\" + YSCMS.Config.UIConfig.dirSite + "\\" + YSCMS.Global.Current.SiteID + "\\" + str_dirFile;
                }
            ...
            foreach (FileInfo DirFile in NewFileInfo)                    //获取此级目录下的所有文件
            {
                if (SelectFile(DirFile.Extension))                       //传入文件后缀名,判断是否是被显示的文件类型,默认显示html,htm,css
                {
                    ...
                    str_picshowadress = YSCMS.Common.Public.GetSiteDomain() + str_picshowadress.Replace("//", "/");
                    if (tf == true)
                        str_TempFileStr += str_TrStart;
        ...
        string PathPre()
        {
            string str_path = YSCMS.Common.FileAction.GetParentPathByPath(str_FilePath + Request.Form["Path"], str_dirFile);
            return str_path;
        }
        ...
        protected void EidtDirName(string path)
        {
            ...
            int result = YSCMS.Common.FileAction.EidtName(path, str_OldName, str_NewName, 0);
            if (result == 1)
        ...
        protected void EidtFileName(string path)
        {
            ...
            int result = YSCMS.Common.FileAction.EidtName(path, str_OldName, str_NewName, 1);
            if (result == 1)
