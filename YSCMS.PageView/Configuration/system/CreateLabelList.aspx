<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CreateLabelList.aspx.cs" Inherits="YSCMS.PageView.Configuration.system.CreateLabelList" %>
...
<script type="text/javascript" language="javascript">
    ...
    function setValue(value) {
        ...
        document.getElementById("UserDefined").value = '{#YS:define=' + value + '}';
    }
...
<div id="dialog-message" title="提示"></div>
         ...
         <script language="javascript" type="text/javascript">
             ...
             function ReturnDivValue() {
                 ...
                 //--------------------返回标签值
                 var temproot = "";
                 var rvalue = "[YS:Loop,YS:SiteID=<%Response.Write(APIID); %>";

                 rvalue += ",YS:NewsType=" + $('#NewsType').val();
                 if ($('#NewsType').val() == "Special") {
                     if ($('#SpecialID').val() != "") { rvalue += ",YS:SpecialID=" + $('#SpecialID').val(); }
                 }
                 else {
                     if ($('#ClassId').val() != "") { rvalue += ",YS:ClassID=" + $('#ClassId').val(); }
                     else { rvalue += ",YS:ClassID=-1"; }
                 }
                 if ($('#Number').val() != "") { rvalue += ",YS:Number=" + $('#Number').val(); }

                 if ($('#ClickNumber').val() != "") { rvalue += ",YS:ClickNumber=" + $('#ClickNumber').val(); }
                 else { rvalue += ",YS:ClickNumber=0"; }

                 if ($('#ShowDateNumer').val() != "") { rvalue += ",YS:ShowDateNumer=" + $('#ShowDateNumer').val(); }
                 else { rvalue += ",YS:ShowDateNumer=365"; }

                 if ($('#Cols').val() != "") { rvalue += ",YS:Cols=" + $('#Cols').val(); }
                 else { rvalue += ",YS:Cols=1"; }

                 if ($('#NewsType').val() == "MarQuee") {
                     if ($('#MarqDirec').val() != "") { rvalue += ",YS:MarqDirec=" + $('#MarqDirec').val(); }
                     else { rvalue += ",YS:MarqDirec=1"; }
                     if ($('#MarqSpeed').val() != "") { rvalue += ",YS:MarqSpeed=" + $('#MarqSpeed').val(); }
                     else { rvalue += ",YS:MarqSpeed=10"; }
                     if ($('#Marqwidth').val() != "") { rvalue += ",YS:Marqwidth=" + $('#Marqwidth').val(); }
                     else { rvalue += ",YS:Marqwidth=10"; }
                     if ($('#Marqheight').val() != "") { rvalue += ",YS:Marqheight=" + $('#Marqheight').val(); }
                     else { rvalue += ",YS:Marqheight=15"; }
                 }

                 if ($('#isPic').val() != "") { rvalue += ",YS:isPic=" + $('#isPic').val(); }
                 else { rvalue += ",YS:isPic=0"; }

                 if ($('#DescType').val() != "") { rvalue += ",YS:DescType=" + $('#DescType').val(); }
                 else { rvalue += ",YS:DescType=1"; }

                 if ($('#Desc').val() != "") { rvalue += ",YS:Desc=" + $('#Desc').val(); }
                 else { rvalue += ",YS:Desc=0"; }

                 if ($('#ShowNavi').val() != "") { rvalue += ",YS:ShowNavi=" + $('#ShowNavi').val(); }
                 else { rvalue += ",YS:ShowNavi=0"; }
                 if ($('#NaviPic').val() != "") { rvalue += ",YS:NaviPic=" + $('#NaviPic').val(); }
                 else { rvalue += ",YS:NaviPic=0"; }

                 if ($('#TitleNumer').val() != "") { rvalue += ",YS:TitleNumer=" + $('#TitleNumer').val(); }
                 else { rvalue += ",YS:TitleNumer=10"; }
                 if ($('#ContentNumber').val() != "") { rvalue += ",YS:ContentNumber=" + $('#ContentNumber').val(); }
                 else { rvalue += ",YS:ContentNumber=200"; }
                 if ($('#NaviNumber').val() != "") { rvalue += ",YS:NaviNumber=" + $('#NaviNumber').val(); }
                 else { rvalue += ",YS:NaviNumber=5"; }
                 if ($('#isSub').val() != "") { rvalue += ",YS:isSub=" + $('#isSub').val(); }
                 else { rvalue += ",YS:isSub=0"; }
                 ...
                 if ($('#Root').val() == "true") {
                     ...
                     temproot = "[#YS:StyleID=" + $('#StyleID').val() + "]";
                 }
                 else {
