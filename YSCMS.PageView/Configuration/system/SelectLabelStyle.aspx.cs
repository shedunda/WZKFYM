...
{
    public partial class SelectLabelStyle : YSCMS.PageBasic.DialogPage
    {
        public SelectLabelStyle()
        {
            BrowserAuthor = EnumDialogAuthority.ForAdmin;
        }
        YSCMS.CMS.RootPublic pd = new CMS.RootPublic();
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (ldt != null)
                    {
                        for (int k = 0; k < ldt.Rows.Count; k++)
                        {
                            ...
                            liststr += "<img src=\"/CSS/imges/folderup.gif\" border=\"0\" class=\"SubItems\" /><a href=\"javascript:void(0);\" onclick=\"getReview1('s" + ldt.Rows[k]["styleID"].ToString() + "')\"><img src=\"/CSS/" + YSCMS.Config.UIConfig.CssPath() + "/imges/review.gif\" title=\"预览\" border=\"0\"></a> <span id=\"" + ldt.Rows[k]["styleID"] + "\" class=\"LableItem\" ondblclick=\"ReturnValue(document.form1.styleID.value);\" onClick=\"sFiles('" + ldt.Rows[k]["styleID"] + "');\" title=\"双击选择||描述：" + ldt.Rows[k]["Description"] + "\">" + ldt.Rows[k]["StyleName"] + "</span><div style=\"background-color:#FFFAE2;display:none;\" id=\"s" + ldt.Rows[k]["styleID"].ToString() + "\">" + tmContent + "</div><br />";
                        }
                    }
                    ldt.Clear();
                    ldt.Dispose();
