...
namespace YSCMS.PageView.Install
{
    public partial class Step3 : System.Web.UI.Page
    {
        ...
        protected string CreateDataBase(string server, string user, string pwd, string dbname, string tbpre)
        {
            ...
            try
            {
                if (File.Exists(path))
                {
                    ...
                    YSCMS.Install.Comm.ExecuteSql(connStr, "master", "IF Not EXISTS (select name from master.dbo.sysdatabases where name = N'" + dbname + "') CREATE DATABASE [" + dbname + "] COLLATE Chinese_PRC_CI_AS");
                    StreamReader reader = File.OpenText(path);
                    string sqlscript = reader.ReadToEnd();
                    reader.Close();
                    sqlscript = this.replaceTablePre(sqlscript, dbname, tbpre);
                    YSCMS.Install.Comm.ExecuteSql(connStr, dbname, sqlscript);
                    YSCMS.Install.Comm.ExecuteSql(connStr, dbname, "CREATE PROCEDURE [" + tbpre + "publish_CHupdateishtml] @tablename varchar(30),@filedname varchar(30),@idtype varchar(30),@id int AS begin declare @sql varchar(500) set @sql='update '+@tablename +' set '+@filedname+'=1 where '+@idtype+'='+'''+@id+''' exec (@sql) end");
                    YSCMS.Install.Comm.ExecuteSql(connStr, dbname, "CREATE PROCEDURE [" + tbpre + "publish_updateishtml] @tablename varchar(30),@filedname varchar(30),@idtype varchar(30),@newsid varchar(12) AS begin declare @sql varchar(500) set @sql='update '+@tablename +' set '+@filedname+'=1 where '+@idtype+'='+''''+@newsid+'''' exec (@sql) end");
                    return str;
                }
                return "数据库脚本文件不存在!";
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string str3 = base.Request.QueryString["yscmsPath"];
        ...
        {
            return Regex.Replace(Regex.Replace(sqlscript, "[Yf][Ss]_", tbpre, RegexOptions.Compiled), "YSCMS", dbname, RegexOptions.Compiled);
        }

        protected string setWebConfig(string yscmsPath, string helpkeyPath, string CollectPath)
        {
            string str = "";
            try
            {
                ...
                foreach (XmlNode node in document["configuration"]["connectionStrings"])
                {
                    if (node.Name == "add")
                    {
                        if (node.Attributes.GetNamedItem("name").Value.ToLower() == "yscms")
                        {
                            node.Attributes.GetNamedItem("connectionString").Value = yscmsPath;
                            flag = true;
                        }
                ...
                foreach (XmlNode node in document["configuration"]["appSettings"])
                {
                    if (node.Name == "add")
                    {
                        ...
                        {
                            node.Attributes.GetNamedItem("value").Value = "YS_";
                            flag = true;
                        }
                        if (node.Attributes.GetNamedItem("key").Value.ToLower() == "webdal")
                        {
                            node.Attributes.GetNamedItem("value").Value = "YSCMS.AccessDAL";
                            flag = true;
                        }
            ...
            try
            {
                ...
                foreach (XmlNode node in document["configuration"]["appSettings"])
                {
                    if (node.Name == "add")
                    {
                        ...
                        if (node.Attributes.GetNamedItem("key").Value.ToLower() == "webdal")
                        {
                            node.Attributes.GetNamedItem("value").Value = "YSCMS.SQLServerDAL";
                            flag = true;
                        }
                        if (node.Attributes.GetNamedItem("key").Value.ToLower() == "mssql")
                        {
