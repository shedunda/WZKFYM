...
{
    public partial class Search : YSCMS.PageBasic.BasePage
    {
        ...
        {
            string SearchTemplet = YSCMS.Publish.General.ReadHtml(GetSearchTemplet());
            ...
            str_List += "<ul>";
            YSCMS.Model.Search si = new YSCMS.Model.Search();
            ...
            try
            {
                DataTable dt = YSCMS.CMS.Search.SearchGetPage(DTable, page, 5, out i, out j, si);
        ...
        protected string getRow(DataRow dr, int ChID)
        {
            ...
            {
                dim = "/" + YSCMS.Config.UIConfig.dirDumm;
            }
            string NewsUrl = "";
            string NewsUrl1 = "";

            FavNewsUrl = YSCMS.Config.UIConfig.dirUser + "/info/collection.aspx?Type=Add&id=" + dr["NewsID"].ToString();
            string SaveClassframe = YSCMS.CMS.Search.GetSaveClassframe(dr["ClassID"].ToString());
            switch (dr["NewsType"].ToString()) //0普通，1图片，2标题
            {
                case "0":
                    NewsUrl = YSCMS.CMS.Search.GetNewsReview(dr["NewsID"].ToString(), "news");
                    ...
                    str_Row += "</p>" + newLine;
                    str_Row += "<p>" + YSCMS.Common.Input.GetSubString(YSCMS.Common.Input.FilterHTML((dr["Content"].ToString()).Replace("?", "？")), 200).Replace(stags, "<span style=\"color:red\">" + stags + "</span>") + "</p>" + newLine;
                ...
                case "1":
                    ...
                    NewsUrl = NewsUrl.Replace("//", "/");
                    NewsUrl1 = "http://" + Request.ServerVariables["SERVER_NAME"] + dim + NewsUrl;
                    string imgr = dim + dr["PicURL"].ToString().ToLower().Replace("{@dirfile}", YSCMS.Config.UIConfig.dirFile);
        ...
        {
            string str_dirMana = YSCMS.Config.UIConfig.dirDumm;
            string str_Templet = YSCMS.Config.UIConfig.dirTemplet;  //获取模板路径
