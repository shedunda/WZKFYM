...
{
    public partial class LogsCreat : YSCMS.PageBasic.UserPage
    {
        YSCMS.CMS.user rd = new YSCMS.CMS.user();
        ...
        protected void Logssubmit(object sender, EventArgs e)
        {
            ...
            {
                //------------------获取表单值-----------------------------------------
                string title = YSCMS.Common.Input.Htmls(this.title.Text);
                string content = YSCMS.Common.Input.Htmls(this.Content.Text);
                ...
                string ramAID;
                ramAID = YSCMS.Common.Rand.Number(12);//产生12位随机字符
                if (Request.Form["log_id"] == "")
                {
                    ...
                    {
                        YSCMS.Model.UserLog1 uc = new YSCMS.Model.UserLog1();
                        uc.LogID = ramAID;
                        uc.title = title;
                        uc.content = content;
                        ...
                        uc.usernum = YSCMS.Global.Current.UserNum;
