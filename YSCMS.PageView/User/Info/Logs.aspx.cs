...
{
    public partial class Logs : YSCMS.PageBasic.UserPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    case "del":            //锁定管理员
                        int ID = int.Parse(YSCMS.Common.Input.Filter(Request.QueryString["ID"]));  //取得需要操作的管理员ID
                        Del(ID);
                        break;
                    default:
                        break;
        ...
        protected void StartLoad(int PageIndex)
        {
            ...
            dt = YSCMS.CMS.Pagination.GetPage("user_manage_logs_1_aspx", PageIndex, 20, out i, out j, st);
