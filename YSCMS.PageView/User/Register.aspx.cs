...
{
    public partial class Register : YSCMS.PageBasic.BasePage
    {
        string Userfiles = YSCMS.Config.UIConfig.UserdirFile;
        new YSCMS.CMS.user User = new YSCMS.CMS.user();
        YSCMS.Model.UserParam upi = new YSCMS.Model.UserParam();
        YSCMS.CMS.News cNew = new CMS.News();
        ...
        protected void storeBut(object sender, ImageClickEventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                ui.Userinfo = "";
                ui.UserFace = "" + YSCMS.Common.Public.GetSiteDomain() + "/Images/user/noHeadpic.gif";
                ...
                else if (User.Add_User(ui) == 1 && User.Add_userfields(ufi) == 1 && User.Add_Ghistory(ughi) == 1)
                {
                    ...
                    YSCMS.Global.Current.Set(new GlobalUserInfo(ui.UserNum, ui.UserName, ui.SiteID, "0"));
                    ...
                    if (upi.returnemail == 1)
                    {
                        ...
                        string activeUrl = YSCMS.Common.Public.GetDomainConfig() + "/" + YSCMS.Config.UIConfig.dirUser + "/info" + "/getPassport.aspx?t=mail&e=" + YSCMS.Common.Input.MD5(ui.Email, true) + "&" + "u=" + YSCMS.Common.Input.MD5(ui.UserNum, true) + "&c=" + ui.EmailCode;
                    ...
                    if (ugi.IsCert == 1)
                    {
                        Response.Write("<script language=\"javascript\" type=\"text/javascript\">alert" +
                                       "('注册成功！\\n但是要求实名认证.\\n点 [确定] 进行实名认证。');" +
                                       "location.href='index.aspx?urls=" + YSCMS.Publish.CommonData.SiteDomain + "/info/userinfo" +
