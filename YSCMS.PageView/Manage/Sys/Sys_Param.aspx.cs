...
{
    public partial class Sys_Param : YSCMS.PageBasic.ManagePage
    {
        public Sys_Param()
        {
            Authority_Code = "Q001";
        ...
        YSCMS.Model.STsys_param su;
        YSCMS.CMS.RootPublic rp = new YSCMS.CMS.RootPublic();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string PageLinkCounts = YSCMS.Common.Public.readparamConfig("PageLinkCount");
                ...
                //设置是否开启分页  默认开启
                string isAutoPageSplit = YSCMS.Config.UIConfig.enableAutoPage;
                ...
                //设置分页字数
                this.txt_pageSplitCount.Text = YSCMS.Config.UIConfig.splitPageCount;
                #endregion
            }
            #endregion
        }
        ...
        void ParamStartLoad()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                #endregion
                BaseCopyRight.Value = YSCMS.Common.Input.ToTxt(dt.Rows[0]["CopyRight"].ToString());
        ...
        void FtpRssStartLoad()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                FTPPASSword.Text = YSCMS.Common.Input.NcyString(dt.Rows[0]["FTPPASSword"].ToString());//字符串解密显示
                #endregion
        ...
        void UserStartLoad()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                RegContent.Value =YSCMS.Common.Input.ToTxt(dt.Rows[0]["RegContent"].ToString());//注册协议
                #endregion
        ...
        protected void SaveBaseInfo_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                su.Str_SiteName = YSCMS.Common.Input.Filter(this.SiteName.Text.Trim());//站点名称
                su.Str_SiteDomain = YSCMS.Common.Input.Filter(this.SiteDomain.Text.Trim());//站点域名
                su.Str_IndexTemplet = YSCMS.Common.Input.Filter(this.IndexTemplet.Text.Trim());//首页摸板路径
                su.Str_Txt_IndexFileName = YSCMS.Common.Input.Filter(this.IndexFileName.Text.Trim());//首页生成文件名
                su.Str_FileEXName = YSCMS.Common.Input.Filter(this.FileEXName.Text.Trim());//默认的扩展名（主站）
                su.Str_ReadNewsTemplet = YSCMS.Common.Input.Filter(this.ReadNewsTemplet.Text.Trim());//新闻浏览页摸板
                su.Str_ClassListTemplet = YSCMS.Common.Input.Filter(this.ClassListTemplet.Text.Trim());//新闻栏目页摸板
                su.Str_SpecialTemplet = YSCMS.Common.Input.Filter(this.SpecialTemplet.Text.Trim());//新闻专题页摸板
                ...
                #endregion
                su.Str_LoginTimeOut = YSCMS.Common.Input.Filter(this.LoginTimeOut.Text.Trim());//后台登陆过期时间
                su.Str_Email = YSCMS.Common.Input.Filter(this.Email.Text.Trim());//管理员信箱
                ...
                su.Str_BaseCopyRight = YSCMS.Common.Input.Filter(this.BaseCopyRight.Value.Trim());//版权信息
                su.Str_CheckInt = YSCMS.Common.Input.Filter(this.CheckInt.Text.Trim());//新闻后台审核机制
                ...
                #endregion
                su.Str_LenSearch = YSCMS.Common.Input.Filter(this.LenSearch.Text.Trim());//关键字长度
                if (YSCMS.Common.Input.IsInteger(this.HistoryNum.Text) == false)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "生成归档索引生成多少天前索引请填写数字", "sys_Param.aspx");
                }
                else
                {
                ...
                if (this.InsertPicPosition.Text.Length < 20)
                {
                    ...
                    {
                        YSCMS.Common.MessageBox.ShowAndRedirect(this, "插入画中画位置格式不正确，格式：插入的位置|right或left", "sys_Param.aspx");
                    }
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "插入画中画位置，长度太长，格式：插入的位置|right或left", "sys_Param.aspx");
                }
                #region 检测栏目标题
                su.chetitle = 0;
                ...
                su.Str_SaveClassFilePath = YSCMS.Common.Input.Filter(this.SaveClassFilePath.Text.Trim());//生成栏目文件保存路径
                ...
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "索引页面规则必须包含{@month}、{@day}及{@year04}或{@year02}", "sys_Param.aspx");
                }
                su.Str_SaveIndexPage = YSCMS.Common.Input.Filter(this.SaveIndexPage.Text.Trim());//生成索引页规则
                su.Str_SaveNewsFilePath = YSCMS.Common.Input.Filter(this.SaveNewsFilePath.Text.Trim());//新闻文件命名规则
                su.Str_SaveNewsDirPath = YSCMS.Common.Input.Filter(this.SaveNewsDirPath.Text.Trim());//新闻文件保存路径
                su.Str_Pram_Index = YSCMS.Common.Input.Filter(this.Pram_Index.Text.Trim());//每页显示数量
                #endregion

                #region 判断
                if (su.Str_SiteName == null || su.Str_SiteName == "" || su.Str_SiteName == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "抱歉，当前基本属性设置中站点名称为空，请填写名称!", "sys_Param.aspx");
                }
                if (su.Str_SiteDomain == null || su.Str_SiteDomain == "" || su.Str_SiteDomain == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "抱歉，当前基本属性设置中站点域名为空，请填写域名!", "sys_Param.aspx");
                }
                #endregion
                ...
                if (sys.Update_BaseInfo(su) != 0)
                {
                    try
                    {
                        YSCMS.Common.Public.SaveXmlConfig("splitPageCount", txt_pageSplitCount, "xml/sys/yscms.config");
                        YSCMS.Common.Public.SaveXmlConfig("enableAutoPage", isAutoPageSplit, "xml/sys/yscms.config");
                    }
                    catch (UnauthorizedAccessException)
                    {
                        YSCMS.Common.MessageBox.ShowAndRedirect(this, "参数保存成功。但更新缓存失败，/xml/sys/yscms.config没有可写权限。", "sys_Param.aspx");

                    }
                    if (YSCMS.Common.Public.saveparamconfig(this.SiteName.Text.Trim(), this.SiteDomain.Text.Trim(), linkTypeConfig, ReviewType, this.IndexTemplet.Text.Trim(), this.IndexFileName.Text.Trim(), this.LenSearch.Text.Trim(), this.SaveIndexPage.Text.Trim(), this.InsertPicPosition.Text, collectTF.ToString(), this.HistoryNum.Text, true, _PageStyle, _PageLinkCount) == false)
                    {
                        YSCMS.Common.MessageBox.ShowAndRedirect(this, "参数保存成功。但更新缓存失败，请检查您的/xml/sys/base.config是否具有可写权限。", "sys_Param.aspx");
                    }
                    //刷新
                    YSCMS.Config.UIConfig.RefurbishCatch();
                    rp.SaveUserAdminLogs(1, 1, UserNum, "保存基本参数设置", "基本参数设置操作成功！");
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "基本参数设置成功", "sys_Param.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "sys_Param.aspx");
                }
                #endregion
            }
        }
        ...
        protected void SaveUser_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                //string Str_IPLimt = YSCMS.Common.Input.Filter(this.IPLimt.Value.Trim());//IP
                su.Str_IPLimt = Request.Form["IPLimt"] + "";
                su.Str_GpointName = YSCMS.Common.Input.Filter(this.GpointName.Text.Trim());//G币
                su.Str_LoginLock = YSCMS.Common.Input.Filter(this.LoginLock.Text.Trim());//锁定
                su.Str_setPoint = YSCMS.Common.Input.Filter(this.setPoint.Text.Trim());//注册获得的积分金币
                su.Str_cPointParam = YSCMS.Common.Input.Filter(this.cPointParam.Text.Trim());//魅力值增加
                su.Str_aPointparam = YSCMS.Common.Input.Filter(this.aPointparam.Text.Trim());//活跃值增加
                su.Str_RegContent = YSCMS.Common.Input.Filter(this.RegContent.Value.Trim());//注册协议
                ...
                {
                    YSCMS.Common.MessageBox.Show(this, "注册项目中，用户名、密码及电子邮件必须选择");
                }
                if (this.returnmobile.SelectedValue == "1" && this.returnemail.SelectedValue == "1")
                {
                    YSCMS.Common.MessageBox.Show(this, "电子邮件认证和手机认证只能选择一个");
                }
                ...
                if (this.returnmobile.SelectedValue == "1")
                {
                    ...
                    {
                        YSCMS.Common.MessageBox.Show(this, "开启了手机验证.注册项目中的手机必须选择!");
                    }
                }
                ...
                if (userpram != 0 && ureg != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "会员参数设置成功", "sys_Param.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "sys_Param.aspx");
        ...
        protected void SaveUpload_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                su.Str_PicServerDomain = YSCMS.Common.Input.Filter(this.PicServerDomain.Text.Trim());//域名
                su.Str_PicUpLoad = YSCMS.Common.Input.Filter(this.PicUpload.Text.Trim());//图片附件目录
                su.Str_UpfilesType = YSCMS.Common.Input.Filter(this.UpfilesType.Text.Trim());//上传格式
                su.Str_UpFilesSize = YSCMS.Common.Input.Filter(this.UpFilesSize.Text.Trim());//上传大小
                ...
                su.Str_RemoteDomain = YSCMS.Common.Input.Filter(this.RemoteDomain.Text.Trim());//远程图片域名
                su.Str_RemoteSavePath = YSCMS.Common.Input.Filter(this.RemoteSavePath.Text.Trim());//远程图片保存路径
                #endregion

                #region 分组刷新
                su.Str_ClassListNum = YSCMS.Common.Input.Filter(this.ClassListNum.Text.Trim());//列表每次刷新数
                su.Str_NewsNum = YSCMS.Common.Input.Filter(this.NewsNum.Text.Trim());//信息每次刷新数
                su.Str_BatDelNum = YSCMS.Common.Input.Filter(this.BatDelNum.Text.Trim());//批量删除数
                su.Str_SpecialNum = YSCMS.Common.Input.Filter(this.SpecialNum.Text.Trim());//专题每次刷新数
                #endregion
                ...
                if (ftpf != 0)
                {
                    if (YSCMS.Common.Public.saveRefreshConfig(this.ClassListNum.Text.Trim(), this.NewsNum.Text.Trim(), this.BatDelNum.Text.Trim(), this.SpecialNum.Text.Trim()) == false)
                    {
                        YSCMS.Common.MessageBox.ShowAndRedirect(this, "上传、分组刷新参数设置成功。但更新缓存失败，请检查您的/xml/sys/refresh.config是否具有可写权限。", "sys_Param.aspx");
                    }
                    rp.SaveUserAdminLogs(1, 1, UserNum, "保存上传，分组刷新设置", "上传，分组刷新设置操作成功！");
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "上传、分组刷新参数设置成功", "sys_Param.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "sys_Param.aspx");
        ...
        protected void SaveJs_ServerClick(object sender, EventArgs e)
        {
            ...
            string Str_JsNews1 = YSCMS.Common.Input.Filter(this.JsNews1.Text.Trim());
            string Str_JsTitle1 = YSCMS.Common.Input.Filter(this.JsTitle1.Text.Trim());
            string Str_JsModel1 = this.JsModel1.SelectedValue;
            su.Str_HotJS = "" + Str_JsNews1 + "|" + Str_JsTitle1 + "|" + Str_JsModel1 + "";

            string Str_JsNews2 = YSCMS.Common.Input.Filter(this.JsNews2.Text.Trim());
            string Str_JsTitle2 = YSCMS.Common.Input.Filter(this.JsTitle2.Text.Trim());
            string Str_JsModel2 = this.JsModel2.SelectedValue;
            su.Str_LastJS = "" + Str_JsNews2 + "|" + Str_JsTitle2 + "|" + Str_JsModel2 + "";

            string Str_JsNews3 = YSCMS.Common.Input.Filter(this.JsNews3.Text.Trim());
            string Str_JsTitle3 = YSCMS.Common.Input.Filter(this.JsTitle3.Text.Trim());
            string Str_JsModel3 = this.JsModel3.SelectedValue;
            su.Str_RecJS = "" + Str_JsNews3 + "|" + Str_JsTitle3 + "|" + Str_JsModel3 + "";

            string Str_JsNews4 = YSCMS.Common.Input.Filter(this.JsNews4.Text.Trim());
            string Str_JsTitle4 = YSCMS.Common.Input.Filter(this.JsTitle4.Text.Trim());
            string Str_JsModel4 = this.JsModel4.SelectedValue;
            su.Str_HoMJS = "" + Str_JsNews4 + "|" + Str_JsTitle4 + "|" + Str_JsModel4 + "";

            string Str_JsNews5 = YSCMS.Common.Input.Filter(this.JsNews5.Text.Trim());
            string Str_JsTitle5 = YSCMS.Common.Input.Filter(this.JsTitle5.Text.Trim());
            ...
            #endregion
            su.Str_FTPIP = YSCMS.Common.Input.Filter(this.FTPIP.Text.Trim());
            su.Str_Ftpport = YSCMS.Common.Input.Filter(this.Ftpport.Text.Trim());
            su.Str_FtpUserName = YSCMS.Common.Input.Filter(this.FtpUserName.Text.Trim());
            su.Str_FTPPASSword = YSCMS.Common.Input.EncryptString(this.FTPPASSword.Text.Trim());//字符串加密方式写入数据库
            #endregion
            #endregion
            ...
            {
                Application.Lock();
                YSCMS.Model.FtpConfig ftpConfigCache = (YSCMS.Model.FtpConfig)Application["FTPInfo"];
            ...
            if (jsj != 0 && ftpp != 0)
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "FTP参数设置成功!", "sys_Param.aspx");
            }
            else
            {
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "sys_Param.aspx");
        ...
        protected void Savewater_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                su.Str_PrintPicTF = YSCMS.Common.Input.Filter(this.PrintPicTF.Text.Trim());//类型
                su.Str_PrintWord = YSCMS.Common.Input.Filter(this.PrintWord.Text.Trim());//文字水印
                su.Str_Printfontsize = YSCMS.Common.Input.Filter(this.Printfontsize.Text.Trim());//字体大小
                su.Str_Printfontfamily = YSCMS.Common.Input.Filter(this.Printfontfamily.Text.Trim());//字体
                su.Str_Printfontcolor = YSCMS.Common.Input.Filter(this.Printfontcolor.Text.Trim());//水印颜色
                su.Str_PrintBTF = YSCMS.Common.Input.Filter(this.PrintBTF.Text.Trim());//文字是否加粗
                su.Str_PintPicURL = YSCMS.Common.Input.Filter(this.PintPicURL.Text.Trim());//图片水印路径
                su.Str_PrintPicsize = YSCMS.Common.Input.Filter(this.PrintPicsize.Text.Trim());//图片水印大小
                su.Str_PintPictrans = YSCMS.Common.Input.Filter(this.PintPictrans.Text.Trim());//透明度
                su.Str_PrintPosition = YSCMS.Common.Input.Filter(this.PrintPosition.Text.Trim());//位置
                su.Str_PrintSmallTF = YSCMS.Common.Input.Filter(this.PrintSmallTF.Text.Trim());//是否开启缩图
                su.Str_PrintSmallSizeStyle = YSCMS.Common.Input.Filter(this.PrintSmallSizeStyle.Text.Trim());//缩图方式
                su.Str_PrintSmallSize = YSCMS.Common.Input.Filter(this.PrintSmallSize.Text.Trim());//缩图大小
                su.Str_PrintSmallinv = YSCMS.Common.Input.Filter(this.PrintSmallinv.Text.Trim());//缩图比例
                ...
                if (watr != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "水印/缩图参数设置成功!", "sys_Param.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误!", "sys_Param.aspx");
        ...
        {
            YSCMS.Common.MessageBox.Show(this,"Here");
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                su.Str_RssNum = YSCMS.Common.Input.Filter(this.RssNum.Text.Trim());//显示范围
                su.Str_RssContentNum = YSCMS.Common.Input.Filter(this.RssContentNum.Text.Trim());//截取数
                su.Str_RssTitle = YSCMS.Common.Input.Filter(this.RssTitle.Text.Trim());//标题
                su.Str_RssPicURL = YSCMS.Common.Input.Filter(this.RssPicURL.Text.Trim());//地址
                ...
                su.Str_WapPath = YSCMS.Common.Input.Filter(WapPaths);//WAP路径
                su.Str_WapDomain = YSCMS.Common.Input.Filter(wapdomains);//WAP域名
                su.Str_WapLastNum = YSCMS.Common.Input.Filter(WapLastNums);//WAP数
                ...
                if (rswa != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "RSS/XML/WAP参数设置成功", "sys_Param.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误!", "sys_Param.aspx");
