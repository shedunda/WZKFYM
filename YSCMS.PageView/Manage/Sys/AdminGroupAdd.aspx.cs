...
{
    public partial class AdminGroupAdd : YSCMS.PageBasic.ManagePage
    {
        ...
        {
            int result = 0;
            YSCMS.Model.AdminGroup agci = new YSCMS.Model.AdminGroup();
            agci.AdminGroupNumber = "";
            agci.GroupName = YSCMS.Common.Input.Filter(Request.Form["GroupName"]);
            agci.ClassList = YSCMS.Common.Input.Filter(Request.Form["News_List"]);
            agci.channelList = YSCMS.Common.Input.Filter(Request.Form["Site_List"]);
            agci.SpecialList = "0";
            agci.SiteID = SiteID;
            ...
            result = agc.add(agci);
            if (result == 1)
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "添加管理员组成功!", "AdminiGroupList.aspx");
            else
                YSCMS.Common.MessageBox.Show(this, "添加管理员组失败!");
        }
    }
}