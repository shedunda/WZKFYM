...
{
    public partial class AdminList : YSCMS.PageBasic.ManagePage
    {
        public AdminList()
        {
            Authority_Code = "Q010";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (Type)
            {
                ...
                case "Del":             //删除管理员
                    ...
                    this.CheckAdminAuthority();
                    Del(YSCMS.Common.Input.checkID(ID));
                    break;
                default:
                    break;
            }
        ...
        protected void StartLoad(int PageIndex)
        {
            ...
            if (site != "" && site != null)
            {
                ...
                dt = YSCMS.CMS.Pagination.GetPage("manage_Sys_admin_list_1_aspx", PageIndex, 20, out i, out j, st);
            }
            else
            {
                dt = YSCMS.CMS.Pagination.GetPage("manage_Sys_admin_list_aspx", PageIndex, 20, out i, out j, null);
            }
            ...
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int k = 0; dt.Rows.Count > k; k++)
                    {
                        ...
                        dt.Rows[k]["userNames"] = "<a class=\"xa1\" href=\"../../" + YSCMS.Config.UIConfig.dirUser + "/showUser.aspx?uid=" + pd.GetUserName(dt.Rows[k]["UserNum"].ToString()) + "\" target=\"_blank\">" + pd.GetUserName(dt.Rows[k]["UserNum"].ToString()) + "</a>";
                    }
                }
        ...
        {
            YSCMS.CMS.Admin ac = new YSCMS.CMS.Admin();
            ac.Lock(ID);
            YSCMS.Common.MessageBox.Show(this, "锁定管理员成功!");
        }
        ...
        protected void UnLock(string ID)
        {
            YSCMS.CMS.Admin ac = new YSCMS.CMS.Admin();
            ac.UnLock(ID);
            YSCMS.Common.MessageBox.Show(this, "解锁管理员成功!");
        }
        ...
        protected void Del(string ID)
        {
            YSCMS.CMS.Admin ac = new YSCMS.CMS.Admin();
            ac.Del(ID);
            YSCMS.Common.MessageBox.Show(this, "删除管理员成功!");
        }
    }
}