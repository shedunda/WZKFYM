...
{
    public partial class AdminPopSet : YSCMS.PageBasic.ManagePage
    {
        public AdminPopSet()
        {
            Authority_Code = "Q015";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    if (ReqUNum == YSCMS.Global.Current.UserNum)
                    {
                        YSCMS.Common.MessageBox.Show(this, "自己给自己设置权限?");
                    }
                    string[] getReturn = rd.GetAdminPopList(YSCMS.Common.Input.checkID(ReqUNum), int.Parse(Id)).Split('|');
                    if (getReturn[0] == "1")
                    {
                        YSCMS.Common.MessageBox.Show(this, "系统管理员不需要设置权限。拥有全部权限!");
                    }
        ...
        protected string ContentPopList(string _xmlPath, string Poplist)
        {
            ...
            try
            {
                ...
                {
                    YSCMS.Common.MessageBox.Show(this, "找不到配置文件(" + _xmlPath + ").可能是虚拟目录配置出错.请修改web.config!");
                }
        ...
        protected string getRoot(string _xmlPath, string flgs)
        {
            ...
            {
                YSCMS.Common.MessageBox.Show(this, "配置文件出错:" + _xmlPath + "!");
            }
            return _Str;
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                rd.UpdatePOPlist(YSCMS.Common.Input.checkID(ReqUNum), int.Parse(YSCMS.Common.Input.checkID(ID)), PopList);
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "更新权限成功!", "AdminList.aspx");
            }
        }
    }
}