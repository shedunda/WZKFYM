...
{
    public partial class FreeLabelAddEnd : YSCMS.PageBasic.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                this.HidName.Value = "{YS_FREE_" + labelnm + "}";
                this.HidID.Value = id.ToString();
                if (fb.IsNameRepeat(id, labelnm))
                {
                    PageError("标签名称重复!", "FreeLabel_List.aspx");
                }
                this.TxtLabelName.Text = "{YS_FREE_" + labelnm + "}";
        ...
        protected void LnkBtnSave_Click(object sender, EventArgs e)
        {
            ...
            content = YSCMS.Common.Input.HtmlDecode(content);
            if (content.Equals(""))
            {
                PageError("标签内容不能为空!", "");
            }
            ...
            YSCMS.Model.FreeLabelInfo info = new YSCMS.Model.FreeLabelInfo(id, sname, lblsql, content, this.TxtDescrpt.Text.Trim());
            YSCMS.CMS.FreeLabel fb = new YSCMS.CMS.FreeLabel();
            ...
            if (id > 0)
            {
                ...
                {
                    //清除缓存
                    YSCMS.Publish.CommonData.DisposeSystemCatch();
        ...
        protected void reviewBtn_Click(object sender, EventArgs e)
        {
            if (reviewBtn.Text == "预览")
            {
                ...
                YSCMS.Model.FreeLabelInfo info = new YSCMS.Model.FreeLabelInfo(id, sname, lblsql, content, this.TxtDescrpt.Text.Trim());
                YSCMS.CMS.FreeLabel fb = new YSCMS.CMS.FreeLabel();
                YSCMS.Publish.FreeLabel flb = new YSCMS.Publish.FreeLabel(sname, YSCMS.Publish.LabelType.Free);
                string htmlStr = "";
                if (id > 0)
                {
                    if (fb.Update(info))
