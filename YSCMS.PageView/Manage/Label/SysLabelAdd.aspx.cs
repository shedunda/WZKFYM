...
{
    public partial class SysLabelAdd : YSCMS.PageBasic.ManagePage
    {
        public SysLabelAdd()
        {
            Authority_Code = "T011";
        ...
        {
            string str_ID = YSCMS.Common.Input.checkID(Request.QueryString["LabelID"]);
            ...
            if (dt != null)
            {
                ...
                LabelName.Value = tLabelName.Replace("{YS_", "").Replace("}", "");
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            else if (Page.IsValid)
            {
                ...
                YSCMS.Model.LabelInfo lbc = new YSCMS.Model.LabelInfo();
                string TmplabelName = this.LabelName.Value;
                string TmplabelName_1 = "";
                if (this.LabelClass.SelectedIndex == 0)
                    TmplabelName_1="{YS_" + TmplabelName.Replace("{", "").Replace("}", "").Replace("Page_", "") + "}";
                else
                    TmplabelName_1 = "{YS_" + TmplabelName.Replace("{", "").Replace("}", "").Replace("S_", "").Replace("Page_", "") + "}";
                if (TmplabelName.ToLower().IndexOf("free_") >= 0)
                {
                    PageError("为了和自由标签区别。自定义标签请不要填写free_等字符!", "");
                }
                if (TmplabelName_1.ToLower().IndexOf("ys_dynclass") > -1 || TmplabelName.ToLower().IndexOf("ys_dynspecial") > -1)
                {
                    PageError("为了和动态标签区别。自定义标签请不要填写DynClass或Special等字符!", "");
                }
                lbc.Label_Name = TmplabelName_1;
                if (this.LabelClass.SelectedIndex == 0)
                    lbc.ClassID = "99999999";
                else
                ...
                YSCMS.CMS.Label labelc = new YSCMS.CMS.Label();
                ...
                if (Request.QueryString["LabelID"] != null && Request.QueryString["LabelID"].ToString() != "")
                {
                    ...
                    {
                        YSCMS.Common.MessageBox.Show(this, "标签名重复！", "提示", "");
                    }
                    else
                    {
                        PageError("修改标签失败!", "");
