...
{
    public partial class SysLabelBak : YSCMS.PageBasic.ManagePage
    {
        public SysLabelBak()
        {
            Authority_Code = "T015";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (str_Op != "" && str_Op != null && str_Op != string.Empty)
            {
                ...
                Rec(YSCMS.Common.Input.checkID(str_ID));
            }
        ...
        {
            int i, j;
            YSCMS.Model.SQLConditionInfo st = new YSCMS.Model.SQLConditionInfo("@SiteID", SiteID);
            DataTable dt = YSCMS.CMS.Pagination.GetPage("manage_label_syslabelbak_aspx", PageIndex, 20, out i, out j, st);
            ...
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        dt.Rows[k]["Op"] = "<a href=\"javascript:Rec('" + dt.Rows[k]["LabelID"].ToString() + "');\">[恢复标签]<a href=\"syslabeladd.aspx?LabelID=" + dt.Rows[k]["LabelID"].ToString() + "\">[修改]</a>";
                    }
                }
