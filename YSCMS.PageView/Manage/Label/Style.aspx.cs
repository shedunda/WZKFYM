...
{
    public partial class Style : YSCMS.PageBasic.ManagePage
    {
        public Style()
        {
            Authority_Code = "T017";
        ...
        protected void StartLoad(int PageIndex)
        {
            ...
            if (action_s != null && action_s != string.Empty)
            {
                ...
                dt = YSCMS.CMS.Pagination.GetPage("manage_label_style_3_aspx", PageIndex, 40, out i, out j, st);
                Cname = "样式名称";
                Back.InnerHtml = "";
            }
            else
            {
                if (str_classid == null || str_classid == "" || str_classid == string.Empty)
                {
                    ...
                    dt = YSCMS.CMS.Pagination.GetPage("manage_label_style_1_aspx", PageIndex, 20, out i, out j, st);
                    tf = true;
                    Cname = "分类名称";
                    Back.InnerHtml = "";
                }
                ...
                {
                    SQLConditionInfo[] st = new SQLConditionInfo[2];
                    st[0] = new SQLConditionInfo("@ClassID", YSCMS.Common.Input.checkID(str_classid));
                    st[1] = new SQLConditionInfo("@SiteID", SiteID);
                    dt = YSCMS.CMS.Pagination.GetPage("manage_label_style_2_aspx", PageIndex, 20, out i, out j, st);
                    Cname = "样式名称";
        ...
        protected void Op()
        {
            ...
            switch (str_Op)
            {
                case "Del":
                    ...
                    {
                        case "style":
                            delstyle(YSCMS.Common.Input.checkID(str_ID));
                            break;
                        case "styleclass":
                            ...
                            {
                                delclass(YSCMS.Common.Input.checkID(str_ID));
                            }
                            break;
                        default:
                            break;
                ...
                case "Dels":
                    ...
                    {
                        case "style":
                            delsstyle(YSCMS.Common.Input.checkID(str_ID));
                            break;
                        case "styleclass":
                            if (ld.getClassLabelCount(str_ID.ToString(), 1) != 0)
                            {
                            ...
                            {
                                delsclass(YSCMS.Common.Input.checkID(str_ID));
                            }
                            break;
                        default:
                            break;
