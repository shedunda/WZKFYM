...
using System.IO;
using YSCMS.NetTransmission;

namespace YSCMS.PageView.Manage.Publish
{
    public partial class FtpSync : System.Web.UI.Page
    {
        protected List<string> queue = YSCMS.Common.Public.GetFTPQueue();
        protected YSCMS.Model.FtpConfig ftpInfo = (Model.FtpConfig)System.Web.HttpContext.Current.Application["FTPInfo"];
        protected void Page_Load(object sender, EventArgs e)
        {
            if (ftpInfo == null) ftpInfo = new YSCMS.Model.FtpConfig();
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            {
                Application.Lock();
                ftpInfo.Operator = YSCMS.Global.Current.UserName;
                Application.UnLock();
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            #region 模板目录
            string[] temp = Directory.GetFiles(Server.MapPath("~/" + YSCMS.Config.UIConfig.dirTemplet), "*", SearchOption.AllDirectories);
            ...
            #region 管理员附件
            temp = Directory.GetFiles(Server.MapPath("~/" + YSCMS.Config.UIConfig.dirFile), "*", SearchOption.AllDirectories);
            for (int c = 0; c < temp.Length; c++)
            {
                if (!queue.Contains(temp[c]))
                {
            ...
            #endregion
            #region 用户附件目录
            temp = Directory.GetFiles(Server.MapPath("~/" + YSCMS.Config.UIConfig.UserdirFile), "*", SearchOption.AllDirectories);
            for (int c = 0; c < temp.Length; c++)
            {
                if (!queue.Contains(temp[c]))
                {
            ...
            #endregion
            #region 文件管理目录
            temp = Directory.GetFiles(Server.MapPath("~/" + YSCMS.Config.UIConfig.filePath), "*", SearchOption.AllDirectories);
            for (int c = 0; c < temp.Length; c++)
            {
                if (!queue.Contains(temp[c]))
                {
