...
{
    public partial class UserGroupAdd : YSCMS.PageBasic.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Request.QueryString["id"] != null && Request.QueryString["id"].Trim() != "")
                {
                    ...
                    try
                    {
                        gid = int.Parse(YSCMS.Common.Input.Filter(Request.QueryString["id"].Trim()));
        ...
        protected void sumbitsave_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                #endregion 获取表单值
                #region 更新数据
                YSCMS.Model.UserInfo4 uc1 = new YSCMS.Model.UserInfo4();
                ...
                {
                    string SessionSiteID = YSCMS.Global.Current.SiteID;
