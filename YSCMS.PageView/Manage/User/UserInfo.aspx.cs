...
{
    public partial class UserInfo : YSCMS.PageBasic.ManagePage
    {
        public UserInfo()
        {
            Authority_Code = "U003";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string uids = YSCMS.Common.Input.Filter(Request.QueryString["id"]);
                int uid = 0;
                try
                {
                    uid = int.Parse(uids);
                ...
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        this.UserInfos.Text = YSCMS.Common.Input.ToTxt(dt.Rows[0]["Userinfo"].ToString());
        ...
        {
            string _Str = "";
            DataTable dt = rd.sexlist(int.Parse(YSCMS.Common.Input.Filter(Request.QueryString["id"])));
            if (dt != null)
            {
        ...
        {
            string _Str = "";
            DataTable dt = rd.marriagelist(int.Parse(YSCMS.Common.Input.Filter(Request.QueryString["id"])));
            if (dt != null)
            {
        ...
        {
            string _Str = "";
            DataTable dt = rd.isopenlist(int.Parse(YSCMS.Common.Input.Filter(Request.QueryString["id"])));
            if (dt != null)
            {
        ...
        protected void submitSave(object sender, EventArgs e)
        {
            if (Page.IsValid == true)                       //判断是否验证成功
            {
                ...
                string Userinfo = this.UserInfos.Text;
        ...
        protected void btnsafe(object sender, EventArgs e)
        {
            if (Page.IsValid == true)                       //判断是否验证成功
            {
                ...
                int suid = int.Parse(YSCMS.Common.Input.Filter(Request.Form["suid"]));
        ...
        protected void btnbase(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                YSCMS.Common.DataCache.DelCache("userinfo-" + Request.QueryString["userNum"]);
                ...
                YSCMS.Model.UserInfo3 uc = new YSCMS.Model.UserInfo3();
                uc.Id = UserID;
