...
{
    public partial class UserGroup : YSCMS.PageBasic.ManagePage
    {
        public UserGroup()
        {
            Authority_Code = "U011";
        }
        YSCMS.CMS.UserMisc cuser = new CMS.UserMisc();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (DelStr == "del")
                {
                    ...
                    try
                    {
                        gId = int.Parse(YSCMS.Common.Input.Filter(Request.QueryString["id"]));
                    }
                    catch (Exception gus)
                    {
