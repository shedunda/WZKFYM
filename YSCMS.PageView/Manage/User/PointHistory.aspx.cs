...
{
    public partial class PointHistory : YSCMS.PageBasic.ManagePage
    {
        public PointHistory()
        {
            Authority_Code = "U028";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (Types)
            {
                case "del":          //删除
                    ...
                    del(YSCMS.Common.Input.Filter(ID.ToString()));
                    break;
                case "PDel":            //批量删除
                    this.Authority_Code = "U029";
                    this.CheckAdminAuthority();
        ...
        protected void StartLoad(int PageIndex, string typep, string UM)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int k = 0; dt.Rows.Count > k; k++)
                {
                    ...
                    if (cuts1 != 0)
                    {
                        dt.Rows[k]["UserName"] = "<a href=\"../../" + YSCMS.Config.UIConfig.dirUser + "/showuser-" + des.Rows[0]["UserName"].ToString() + ".aspx\" target=\"_blank\">" + des.Rows[0]["UserName"].ToString() + "</a>";
                    }
                    else
                    {
