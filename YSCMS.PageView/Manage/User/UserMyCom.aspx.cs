...
{
    public partial class UserMyCom : YSCMS.PageBasic.ManagePage
    {
        public UserMyCom()
        {
            Authority_Code = "U034";
        ...
        protected void StartLoad(int PageIndex, string GoodTitle2, string UserID, string title, string Um, string dtm1, string dtm2, string isCheck, string islock)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow h in dt.Rows)
                {
                    ...
                    {
                        h["UserNames"] = "<a href=\"../../" + YSCMS.Config.UIConfig.dirUser + "/showuser-" + h["UserNum"].ToString() + ".aspx\" target=\"_blank\" title=\"查看此用户信息\">" + h["UserNum"].ToString() + "</a>";
                    }
                    else
                    {
