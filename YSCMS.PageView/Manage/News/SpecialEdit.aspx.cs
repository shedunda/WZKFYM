...
{
    public partial class SpecialEdit : YSCMS.PageBasic.ManagePage
    {
        public SpecialEdit()
        {
            Authority_Code = "C039";
        ...
        {
            string ID = YSCMS.Common.Input.checkID(Request.QueryString["ID"]);

            YSCMS.Model.NewsSpecial NewsSpecialModel = NewsSpecialCMS.GetModel(ID);
            ...
            {
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "参数传递错误!", "SpecialList.aspx");
            }
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string fileNames = YSCMS.CMS.CommStr.FileRandName(Request.Form["S_FileRule"]);      //文件名生成规则
            string f_name = Regex.Replace(fileNames, "{@EName}", enNames);

            NewsSpecialModel.saveDirPath = YSCMS.CMS.CommStr.FileRandName(Request.Form["S_DirRule"]);    //目录生成规则
            ...
            if (result)
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "修改专题信息成功!", "SpecialList.aspx");
            }
            else
                YSCMS.Common.MessageBox.Show(this, "修改专题信息失败!");
        }
