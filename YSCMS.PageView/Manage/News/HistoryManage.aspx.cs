...
{
    public partial class HistoryManage : YSCMS.PageBasic.ManagePage
    {
        public HistoryManage()
        {
            Authority_Code = "C048";
        }
        YSCMS.CMS.News ns = new YSCMS.CMS.News();
        ...
        {
            int i, j;
            DataTable dt = YSCMS.CMS.Pagination.GetPage("manage_news_History_Manage_aspx", PageIndex, 20, out i, out j, null);
        ...
        protected void Del_ClickP(object sender, EventArgs e)
        {
            ...
            {
                YSCMS.Common.MessageBox.Show(this, "请先选择删除操作的新闻!", "提示", "");
            }
            else
            {
                if (ns.DelOld(Checkbox1) == 0)
                {
                    ...
                    YSCMS.Common.MessageBox.Show(this, "新闻数据删除失败,请与管理联系!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
                }
                else
                {
                    log.SaveUserAdminLogs(1, 1, UserNum, "新闻数据删除成功", "新闻数据删除成功");
                    YSCMS.Common.MessageBox.Show(this, "新闻数据删除成功!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
                }
            }
        }
        /// <summary>
        ...
        protected void Suo_ClickP(object sender, EventArgs e)
        {
            ...
            if (Checkbox1 == null || Checkbox1 == string.Empty)
            {
                YSCMS.Common.MessageBox.Show(this, "请先选择锁定操作的新闻!", "提示", "");
            }
            else
            {
                if (ns.UpdateOld(Checkbox1, 1, "1", "isLock") == 0)
                {
                    YSCMS.Common.MessageBox.Show(this, "新闻数据锁定失败,请与管理联系!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
                }
                else
                {
                    log.SaveUserAdminLogs(1, 1, UserNum, "新闻数据锁定成功", "新闻数据锁定成功");
                    YSCMS.Common.MessageBox.Show(this, "新闻数据锁定成功!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
                }
            }
        }
        /// <summary>
        ...
        protected void Unsuo_ClickP(object sender, EventArgs e)
        {
            ...
            if (Checkbox1 == null || Checkbox1 == string.Empty)
            {
                YSCMS.Common.MessageBox.Show(this, "请先选择解锁操作的新闻!", "提示", "");
            }
            else
            {
                if (ns.UpdateOld(Checkbox1, 1, "0", "isLock") == 0)
                {
                    YSCMS.Common.MessageBox.Show(this, "新闻数据解锁失败,请与管理联系!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
                }
                {
                    log.SaveUserAdminLogs(1, 1, UserNum, "新闻数据解锁成功", "新闻数据解锁成功");
                    YSCMS.Common.MessageBox.Show(this, "新闻数据解锁成功!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
                }
            }
        }
        /// <summary>
        ...
        protected void Index_ClickP(object sender, EventArgs e)
        {
            ...
            int getHistoryNum = int.Parse(YSCMS.Common.Public.readparamConfig("HistoryNum"));
            try
            {
                YSCMS.Common.HProgressBar.Roll("正在发布索引", 0);
                int m = 0;
                int j = 0;
                for (int i = 0; i < getHistoryNum; i++)
                {
                    if (YSCMS.Publish.General.PublishHistryIndex(i))
                    {
                        m++;
                    }
                    else
                    ...
                    YSCMS.Common.HProgressBar.Roll("正在发布第" + i + "天,共" + getHistoryNum + ",失败" + j + "个(可能当天没归档新闻。)", ((i + 1) * 100 / getHistoryNum));
                }
                YSCMS.Common.HProgressBar.Roll("发布索引成功, 共" + getHistoryNum + ",失败" + j + "个(可能当天没归档新闻。). &nbsp;<a href=\"history_Manage.aspx\">返回</a>", 100);
            }
            catch (Exception ex)
            {
                YSCMS.Common.Public.savePublicLogFiles("□□□发布索引", "【错误描述：】\r\n" + ex.ToString(), UserName);
                YSCMS.Common.HProgressBar.Roll("发布索引失败。<a href=\"error/geterror.aspx?\">查看日志</a>", 0);
            }
            Response.End();
        }
        ...
        protected void DelAll_ClickP(object sender, EventArgs e)
        {
            ...
            if (delap == 0)
            {
                ...
                YSCMS.Common.MessageBox.Show(this, "新闻数据全部删除失败,请与管理联系!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
            }
            log.SaveUserAdminLogs(1, 1, UserNum, "删除全部归档新闻", "新闻数据全部删除成功");
            YSCMS.Common.MessageBox.Show(this, "新闻数据全部删除成功!", "提示", "__doPostBack('PageNavigator1$LnkBtnGoto', '');");
        }
