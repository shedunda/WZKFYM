...
{
    public partial class NewsList : YSCMS.PageBasic.ManagePage
    {
        public NewsList()
        {
            Authority_Code = "C000";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (_TClassID != null)
                {
                    if (YSCMS.Config.verConfig.PublicType != "1")
                    {
        ...
        {
            if (YSCMS.Publish.General.PublishXML(sid))
            {
        ...
        private void Option_makeFilesHTML(string sid)
        {
            ...
            if (id.IndexOf(",") == -1)
            {
                ...
                if (publishType == "0")
                {
                    if (YSCMS.Publish.General.PublishSingleNews(NewsID_1, ClassID_1))
                    {
            ...
            else
            {
                ...
                for (int i = 0; i < idARR.Length; i++)
                {
                    ...
                    if (publishType == "0")
                    {
                        if (YSCMS.Publish.General.PublishSingleNews(sNewsID, sClassID))
                        {
