...
{
    public partial class Frame : YSCMS.PageBasic.ManagePage
    {
        YSCMS.CMS.News cNews = new YSCMS.CMS.News();
        YSCMS.CMS.NewsJS Cjs = new NewsJS();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Request.QueryString["Special"] == null)
                {
                    ...
                    DataTable TB = Cjs.GetList("SiteID='" + YSCMS.Global.Current.SiteID + "' and jsType=1");
                    if (TB != null)
                    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    DataTable dt = cNews.GetNewsConent("NewsID,NewsTitle,PicURL,ClassID,CreatTime", "SiteID='" + YSCMS.Global.Current.SiteID + "' and NewsID='" + ID[i] + "'", "");
                    if (dt != null)
                    {
                    ...
                    YSCMS.Model.NewsJSFile mnewsjs = new Model.NewsJSFile();
                    YSCMS.CMS.NewsJSFile cnewsjs = new NewsJSFile();
                    mnewsjs.JsID = JsID;
                ...
                //YSCMS.Model.NewsJS info = nj.GetModel(JsID);
                //nj.Update(info);
            }
