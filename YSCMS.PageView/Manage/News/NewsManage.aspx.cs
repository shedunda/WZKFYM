...
{
    public partial class NewsManage : YSCMS.PageBasic.ManagePage
    {
        YSCMS.CMS.News td = new CMS.News();
        private DataTable TbClass;
        private string OriginalType;
        private string sOrgNews = "";
        YSCMS.CMS.sys csys = new CMS.sys();
        ...
        private void BindNews()
        {
            ...
            string id = YSCMS.Common.Input.Filter(this.LblIDs.Text.Trim());
            if (!id.Equals(""))
            {
                string s = YSCMS.Common.Input.Filter(this.LblIDs.Text);
                s = "'" + s.Replace(",", "','") + "'";
                DataTable tb = null;
        ...
        {
            TbClass = cClass.GetContent("ClassID,ClassCName,ParentID", "isURL=0 and isLock=0 and isRecyle=0 and SiteID='" + YSCMS.Global.Current.SiteID + "'", "");
            this.LstOriginal.Items.Clear();
        ...
        protected void NewsCopy()
        {
            ...
            {
            ID:
                string NewsID = YSCMS.Common.Rand.Number(12);
                if (td.Exists(NewsID)) { goto ID; }
                string _FileName = YSCMS.Common.Rand.Number(5);
        ...
        protected void ClassCopy()
        {
            ...
            string[] soclass = YSCMS.Common.Public.Lost(str).Split(',');
        ...
        protected void pro_click(object sender, EventArgs e)
        {
            ...
            else if (OriginalType.Equals("1"))//对选中的栏目进行操作
            {
                ...
                string soclass = YSCMS.Common.Public.Lost(str);
