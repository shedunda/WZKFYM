...
{
    public partial class SpecialList : YSCMS.PageBasic.ManagePage
    {
        public SpecialList()
        {
            Authority_Code = "C038";
        }
        YSCMS.CMS.NewsSpecial NewsSpecialCMS = new YSCMS.CMS.NewsSpecial();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (Type)
            {
                case "Lock":            //锁定专题
                    ...
                    Lock(YSCMS.Common.Input.checkID(ID));
                    break;
                case "UnLock":          //解锁专题
                    this.Authority_Code = "C041";
                    this.CheckAdminAuthority();
                    UnLock(YSCMS.Common.Input.checkID(ID));
                    break;
                case "PDel":            //批量删除专题
        ...
        protected void StartLoad(int PageIndex, bool IsSearch)
        {
            ...
            {
                dt = NewsSpecialCMS.GetPage(YSCMS.Global.Current.SiteID, YSCMS.Config.UIConfig.GetPageSize(), PageIndex, out RecordCount, out PageCount);
            }
        ...
        protected void Lock(string ID)
        {
            ...
            YSCMS.Common.MessageBox.ShowAndRedirect(this, "锁定专题成功!", "SpecialList.aspx");
        ...
        {
            NewsSpecialCMS.SetLock(ID);
            YSCMS.Common.MessageBox.ShowAndRedirect(this, "解锁专题成功!", "SpecialList.aspx");
        }
        ...
        protected void PDel(string mode)
        {
            if (Request.Form["S_ID"] == null || Request.Form["S_ID"] == "")
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "请选择要批量删除的专题!", "SpecialList.aspx");
            string SpecialID = Request.Form["S_ID"];
            SpecialID = YSCMS.Common.Input.Losestr(SpecialID);
            if (SpecialID == "IsNull")
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "请选择要批量删除的专题!", "SpecialList.aspx");
            ...
            if (mode == "Re")
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "将专题删除到回收站成功!", "SpecialList.aspx");
            }
            else
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "彻底删除成功!", "SpecialList.aspx");
            }
        }
        ...
        protected void Plock()
        {
            ...
            str_SID = YSCMS.Common.Input.Losestr(str_SID);
            if (str_SID == "IsNull")
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "请选择要批量锁定的专题!", "SpecialList.aspx");
            NewsSpecialCMS.SetLock(str_SID);
            YSCMS.Common.MessageBox.ShowAndRedirect(this, "批量锁定成功!", "SpecialList.aspx");
        }
        ...
        protected void PUnlock()
        {
            ...
            SpecialId = YSCMS.Common.Input.Losestr(SpecialId);
            if (SpecialId == "IsNull")
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "请选择要批量解锁的专题!", "SpecialList.aspx");

            NewsSpecialCMS.SetLock(SpecialId);
            YSCMS.Common.MessageBox.ShowAndRedirect(this, "批量解锁成功!\n如果批量选中的专题还有未解锁的,请先解锁此专题的父专题!", "SpecialList.aspx");
        }
        ...
        protected void Publish()
        {
            ...
            str_SID = YSCMS.Common.Input.Losestr(str_SID);
            if (str_SID == "IsNull")
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "请选择要生成的专题!", "SpecialList.aspx");
            ...
            try
            {
                YSCMS.Common.HProgressBar.Roll("正在发布专题", 0);
                int j = 0;
                ...
                for (int i = 0; i < m; i++)
                {
                    ...
                    YSCMS.Common.HProgressBar.Roll("共生成" + m + "个专题，正在发布" + (i + 1) + "个。", ((i + 1) * 100 / m));
                }
                YSCMS.Common.HProgressBar.Roll("发布专题成功,成功" + j + "个,<a href=\"../Publish/error/geterror.aspx?\">失败" + (arr_SID.Length - j) + "个(可能有专题有浏览权限)</a>. &nbsp;<a href=\"speciallist.aspx\">返回</a>", 100);
            }
            catch (Exception ex)
            {
                YSCMS.Common.Public.savePublicLogFiles("□□□发布专题", "【错误描述：】\r\n" + ex.ToString(), UserName);
                YSCMS.Common.HProgressBar.Roll("发布专题失败。<a href=\"../publish/error/geterror.aspx?\">查看日志</a>", 0);
            }
            Response.End();
