...
{
    public partial class NewsAdd : YSCMS.PageBasic.ManagePage
    {
        protected String unNewsid = "";
        YSCMS.Model.News mnews = new Model.News();
        YSCMS.CMS.News cnews = new CMS.News();
        YSCMS.CMS.NewsClass cclass = new CMS.NewsClass();
        YSCMS.Model.NewsClass mclass = new Model.NewsClass();
        YSCMS.CMS.RootPublic rd = new CMS.RootPublic();
        ...
        YSCMS.CMS.sys param = new CMS.sys();
        YSCMS.CMS.DropTemplet DropTempletCMS = new DropTemplet();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    this.CheckBox1.Checked = bool.Parse(YSCMS.Config.UIConfig.enableAutoPage);
                }
                catch
                {
                ...
                try
                {
                    int i = Int32.Parse(YSCMS.Config.UIConfig.splitPageCount);
                ...
                if (!string.IsNullOrEmpty(_EditAction))
                {
                    if (_EditAction.ToString() == "Edit")
                    {
                        ...
                        #region 得到栏目数据表
                        string _DataLib = YSCMS.Config.UIConfig.dataRe + "news";
                        #endregion 得到数据表结束
        ...
        protected void GetunNewsData()
        {
            ...
            {
                #region 编辑不规则新闻
                unNewsid = YSCMS.Common.Input.Filter(unNewsid);
                    ...
                    if ((dt.Rows[0]["Defineworkey"].ToString()) != string.Empty)
                    {
                        ...
                        for (int i = 0; i < dkARR.Length; i++)
                        {
                            ...
                            if (dts != null && dts.Rows.Count > 0)
                            {
                                ...
                                string rdnum = YSCMS.Common.Rand.Number(4, true);
                                switch (typeFlg)
                                {
                                    case "1":
        ...
        protected void Buttonsave_Click(object sender, EventArgs e)
        {
            ...
            {
            RandFirst: NewsID = YSCMS.Common.Rand.Number(12);
                #region 检查编号是否存在
            ...
            if (this.sNaviContentFromContent.Checked)
            {
                ...
                string LostResultStr = YSCMS.Common.Input.LostHTML(naviContent);
                LostResultStr = YSCMS.Common.Input.LostPage(LostResultStr);
                LostResultStr = YSCMS.Common.Input.LostVoteStr(LostResultStr);
                naviContent = YSCMS.Common.Input.GetSubString(LostResultStr, 255);
            }
            ...
            if (enableAutoPage)
            {
                try
                {
                    Content = YSCMS.Common.Input.AutoSplitPage(Content, int.Parse(this.TxtPageCount.Text));
                }
                catch (Exception ex)
                {
                    //Content = YSCMS.Common.Input.AutoSplitPage(Content,20);
                }
            }
            if (EditAction != "Edit")
            {
                if (YSCMS.Config.UIConfig.isLinkTF == "1")
                {
            ...
            YSCMS.Model.News uc = new YSCMS.Model.News();
            ...
            if (SPicURLTF == 1)
            {
                ...
                string _tmpPic = "/" + localSavedir + "/shortPictures/" + getDateTime.Year + "-" + getDateTime.Month + "/" + getDateTime.Year + getDateTime.Month + getDateTime.Day + YSCMS.Common.Rand.Number(5) + fileEX;
                ...
                int _sWidth = int.Parse(YSCMS.Config.UIConfig.sWidth);
                int _sHeight = int.Parse(YSCMS.Config.UIConfig.sHeight);
                ...
                YSImage FSI = new YSImage(_sWidth, _sHeight, _PicURL);
                //设置比例
                string _FSISmallin = null;
                YSCMS.CMS.sys syss = new YSCMS.CMS.sys();
            ...
            {
                _IDStr = int.Parse(YSCMS.Common.Rand.Number(8));
            }
            #endregion
            ...
            if (NewsProperty_TTTF1 == "1")
            {
                ...
                {
                    uc.NewsPicTopline = 1;
                    YSCMS.Model.NewsContentTT uc1 = new YSCMS.Model.NewsContentTT();
                    uc1.NewsTF = 0;
                    uc1.NewsID = NewsID;
                    ...
                    if (EditAction == "Edit")
                    {
                        ...
                        {
                            uc1.tl_SavePath = "/{@dirFile}/topline/" + getDateTime.Year + "-" + getDateTime.Month + "/" + YSCMS.Common.Rand.Number(15) + ".jpg";
                        }
                    ...
                    YSImage FSI = new YSImage(int.Parse(this.PagePicwidth.Text), 0, _Tmp_SavePath);
            ...
            YSCMS.CMS.DefineData cdef = new YSCMS.CMS.DefineData();
            ...
            if (SubTF.Checked)
            {
                String OldNewsId = YSCMS.Common.Input.Filter(Request.Form["NewsIDs"]);
                ...
                if (this.isHTML.Checked)
                {
                    if (ReadType == "0")
                    {
                        ...
                        else
                        {
                            if (isLock == 0)
                            {
                                YSCMS.CMS.sys param = new YSCMS.CMS.sys();
                                string publishType = param.GetParamBase("publishType");

                                if (publishType == "0")
                                {
                                    if (YSCMS.Publish.General.PublishSingleNews(NewsID, ClassID, isCom))
                                    {
                                ...
                                else
                                {
                                    ...
                                    if (dt != null && dt.Rows.Count > 0)
                                    {
                                        YSCMS.Publish.DropPublish dp = new YSCMS.Publish.DropPublish();
            ...
            string[] publicfreshinfoARR = YSCMS.Config.UIConfig.publicfreshinfo.Split('|');
            string publicclass = publicfreshinfoARR[0].ToString();
            string publicspecial = publicfreshinfoARR[1].ToString();
            if (publicclass == "1")
            {
                YSCMS.Publish.General pn = new YSCMS.Publish.General();
            ...
            //清除缓存
            //YSCMS.Publish.CommonData.NewsInfoList.Dispose();
            YSCMS.Publish.CommonData.NewsInfoList = null;
        ...
        {
            YSCMS.CMS.Collect.RemoteResource red = new YSCMS.CMS.Collect.RemoteResource(_Content, _localSavedir, _PhylocalSavedir, "", ReminTF);
