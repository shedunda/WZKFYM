    ...
    public partial class Left : YSCMS.PageBasic.ManagePage
    {
        ...
        string getlist()
        {
            ...
            if (site.Rows.Count > 0)
            {
                ...
                {
                    content += "<li  class=\"open\"><img src=\"/CSS/" + YSCMS.Config.UIConfig.CssPath() + "/imges/lie_26.gif\" border=\"0\" /><a target=\"sys_main\" href=\"news/NewsList.aspx\">" + item["CName"].ToString() + "</a><a href=\"javascript:location.reload()\">(刷新)</a>";
        ...
        string list(string siteid, string classid)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow item in dt.Rows)
                {
                    ...
                    {
                        content += "<li><img src=\"/CSS/" + YSCMS.Config.UIConfig.CssPath() + "/imges/lie_26.gif\" border=\"0\" /><a target=\"sys_main\" href=\"news/NewsList.aspx?ClassID=" + item["ClassID"].ToString() + "\">" + item["ClassCName"].ToString() + "(<span style=\"color:red\">" + count + "</span>)</a><a target=\"sys_main\" href=\"news/newsAdd.aspx?ClassID=" + item["ClassID"].ToString() + "&EditAction=Add\" title=\"添加此栏目下的内容\"><img src=\"imges/lie_78.gif\" border=\"0\" /></a>";
                        content += list(siteid, item["ClassID"].ToString());
                        content += "</li>";
                    }
                    else
                    {
                        content += "<li><img src=\"/CSS/" + YSCMS.Config.UIConfig.CssPath() + "/imges/lie_26.gif\" border=\"0\" />" + item["ClassCName"].ToString() + "(<span style=\"color:red\">" + count + "</span>)";
                        content += list(siteid, item["ClassID"].ToString());
                        content += "</li>";
                    }
                }
