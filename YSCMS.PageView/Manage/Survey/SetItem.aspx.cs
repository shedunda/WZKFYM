...
{
    public partial class SetItem : YSCMS.PageBasic.ManagePage
    {
        public SetItem()
        {
            Authority_Code = "S004";
        ...
        protected void VoteItemManage(int PageIndex)//显示投票选项管理页面
        {
            ...
            string KeyWord = YSCMS.Common.Input.Filter(this.KeyWord.Text.Trim());//关键字
            ...
            if (KeyWord != "" && KeyWord != null)//如果关键字不为空，则执行下面的条件语句
            {
                switch (type)
                {
                        ...
                        if (dt1 != null)
                        {
                            if (dt1.Rows.Count > 0)
                            {
                                for (int l = 0; l < dt1.Rows.Count; l++)
                                {
                                    ...
                                    dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_1_aspx", PageIndex, num, out i, out j, st);
                                }
                            }
                            else
                            {
                                dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_2_aspx", PageIndex, num, out i, out j, null);
                            }
                        }
                        else
                        {
                    ...
                    case "ItemNamee":
                        ...
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_3_aspx", PageIndex, num, out i, out j, st1);
                        break;
                    case "ItemValuee":
                        SQLConditionInfo st2 = new SQLConditionInfo("@ItemValue", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_4_aspx", PageIndex, num, out i, out j, st2);
                        break;
                    case "PicSrcc":
                        SQLConditionInfo st3 = new SQLConditionInfo("@PicSrc", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_5_aspx", PageIndex, num, out i, out j, st3);
                        break;
                    case "DisColorr":
                        SQLConditionInfo st4 = new SQLConditionInfo("@DisColor", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_6_aspx", PageIndex, num, out i, out j, st4);
                        break;
                    case "VoteCountt":
                        SQLConditionInfo st5 = new SQLConditionInfo("@VoteCount", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_7_aspx", PageIndex, num, out i, out j, st5);
                        break;
                    case "ItemDetaill":
                        SQLConditionInfo st6 = new SQLConditionInfo("@ItemDetail", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setItem_8_aspx", PageIndex, num, out i, out j, st6);
                        break;
                }
            }
            else
        ...
        protected void Saveitem_ServerClick(object sender, EventArgs e)
        {
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                string Str_ItemName = YSCMS.Common.Input.Filter(this.ItemName.Text);//选项描述
                ...
                string Str_DisColor = YSCMS.Common.Input.Filter(this.DisColor.Text);//颜色
                string Str_VoteCount = YSCMS.Common.Input.Filter(this.VoteCount.Text);//票数
                string Str_ItemDetail = this.ItemDetail.Value;//详细说明

                //检查是否有已经存在的类别名称
                if (sur.Str_CheckSql(Str_ItemName) != 0)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，该选项已经存在", "setItem.aspx");
                }
                //检查选项名不能为空
                if (Str_ItemName == null || Str_ItemName == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，选项名不能为空!", "setItem.aspx");
                }
                //检查选项票数是否为数字型
                if (!Common.Input.IsInteger(Str_VoteCount))
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，选项票数只能为数字型，请返回继续添加", "setItem.aspx");
                }
                //向数据库中写入添加的类别信息
                ...
                if (sur.Add_Str_InSql(Str_vote_CTName, Str_ItemName, Str_ItemValue, Str_ItemMode, Str_PicSrc, Str_DisColor, Str_VoteCount, Str_ItemDetail, SiteID) != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "问卷调查系统新增选项成功", "setItem.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "setItem.aspx");
                }
        ...
        protected void DelP_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除数据成功,请返回继续操作!", "setItem.aspx");
            }
        }
        ...
        protected void DelAll_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除全部成功。", "setItem.aspx");
            }
        }
        ...
        protected void Editclick_ServerClick(object sender, EventArgs e)
        {
            if (Page.IsValid)//判断页面是否通过验证
            {
                int IID = int.Parse(Request.QueryString["ID"]);
                //取得修改中的表单信息
                string Str_classnameedit = YSCMS.Common.Input.Filter(this.classnameedit.Text.Trim());
                string Str_itemnameedit = YSCMS.Common.Input.Filter(this.itemnameedit.Text.Trim());
                string Str_valueedit = YSCMS.Common.Input.Filter(this.valueedit.Text.Trim());
                string Str_itemmodele = YSCMS.Common.Input.Filter(this.itemmodele.Text.Trim());
                string Str_picsurl = YSCMS.Common.Input.Filter(this.picsurl.Text.Trim());
                string Str_discoloredit = YSCMS.Common.Input.Filter(this.discoloredit.Text.Trim());
                string Str_pointqe = YSCMS.Common.Input.Filter(this.pointqe.Text.Trim());
                string Str_discriptionitem = YSCMS.Common.Input.Filter(this.discriptionitem.Value.Trim());

                if (Str_itemnameedit == null || Str_itemnameedit == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，主题名称不能为空，请返回继续添加", "setItem.aspx");
                }
                //检查选项数是否为数字型
                ...
                if (sur.Update_Str_UpdateSql(Str_classnameedit, Str_itemnameedit, Str_valueedit, Str_itemmodele, Str_picsurl, Str_discoloredit, Str_pointqe, Str_discriptionitem, IID) != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "修改成功", "setItem.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "setItem.aspx");
