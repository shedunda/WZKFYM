...
{
    public partial class SetTitle : YSCMS.PageBasic.ManagePage
    {
        public SetTitle()
        {
            Authority_Code = "S004";
        ...
        void setTitleEdit()
        {
            try
            {
                ...
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "未知错误,异常错误!", "setTitle.aspx");
                }
            }
            catch { }
        }
        ...
        void setTitleDel()
        {
            ...
            else
            {
                ...
                if (v1 && v2 && v3 && v4)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除成功!", "setTitle.aspx");
                }
                else
                {
        ...
        {
            #region 查询条件语句判断
            string KeyWord = YSCMS.Common.Input.Filter(this.KeyWord.Text.Trim());//取得关键字的内容
            ...
            if (KeyWord != "" && KeyWord != null)//如果关键字处内容不为空,则按下面选择语句进行条件判断并传值
            {
                ...
                {
                    case "choose":
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_1_aspx", PageIndex, num, out i, out j, null);//不跟条件进行查询，显示全部内容                   
                        break;
                    case "title":
                        SQLConditionInfo st = new SQLConditionInfo("@Title", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_2_aspx", PageIndex, num, out i, out j, st);//按照主题名查询相关内容                   
                        break;
                    case "class":
                        ...
                        if (dt1 != null)
                        {
                            if (dt1.Rows.Count > 0)
                            {
                                for (int l = 0; l < dt1.Rows.Count; l++)
                                {
                                    ...
                                    dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_3_aspx", PageIndex, num, out i, out j, st1);//按照类名查询                              
                                }
                            }
                            else
                            {
                                dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_1_aspx", PageIndex, num, out i, out j, null);
                            }
                        }
                        else
                        {
                    ...
                    case "num":
                        ...
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_4_aspx", PageIndex, num, out i, out j, st2);//按照最多数查询
                        break;
                    case "starttime":
                        SQLConditionInfo st3 = new SQLConditionInfo("@StartDate", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_5_aspx", PageIndex, num, out i, out j, st3);//按照开始时间查询
                        break;
                    case "endtime":
                        SQLConditionInfo st4 = new SQLConditionInfo("@EndDate", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_6_aspx", PageIndex, num, out i, out j, st4);//按照结束时间查询
                        break;
                    case "itemmodel":
                        SQLConditionInfo st5 = new SQLConditionInfo("@ItemMode", "%" + KeyWord + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_7_aspx", PageIndex, num, out i, out j, st5); //按照排列方式查询
                        break;
                    default:
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setTitle_1_aspx", PageIndex, num, out i, out j, null);
                        break;
        ...
        protected void DelP_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除数据成功,请返回继续操作!", "setTitle.aspx");
            }
        }
        ...
        protected void Savetitle_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                ...
                string Str_Title = YSCMS.Common.Input.Filter(this.Title.Text);//类别名称
                string Str_TypeSelect = this.TypeSelect.SelectedValue;//项目类型
                string Str_MaxselectNum = YSCMS.Common.Input.Filter(this.MaxselectNum.Text);//最多选项个数
                string Str_DisModel = this.DisModel.SelectedValue;//显示方式
                string Str_Starttime = YSCMS.Common.Input.Filter(this.Starttime.Text);//开始时间
                string Str_Endtime = YSCMS.Common.Input.Filter(this.Endtime.Text);//结束时间
                ...
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，该主题已经存在!", "setTitle.aspx?type=add");
                }
                //检查主题名是否为空
                if (Str_Title == null || Str_Title == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，主题名称不能为空，请返回继续添加!", "setTitle.aspx");
                }
                //检查选项数是否为数字型
                ...
                else//否则，是多选，则若输入不是数字,则出错,反之成功
                {
                    ...
                    {
                        YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，选项数只能为数字型，请返回继续添加!", "setTitle.aspx");
                    }
                    else
                    {
                        //将输入的值赋给最多选项数
                        Str_MaxselectNum = YSCMS.Common.Input.Filter(this.MaxselectNum.Text);
                    }
                }
                //检查日期是否合法
                if (YSCMS.Common.Input.ChkDate(Str_Starttime) == false)
                {
                    YSCMS.Common.MessageBox.Show(this, "对不起，选项数只能为数字型，请返回继续添加!");
                    return;
                }
                if (YSCMS.Common.Input.ChkDate(Str_Endtime) == false)
                {
                    YSCMS.Common.MessageBox.Show(this, "对不起，选项数只能为数字型，请返回继续添加!");
                    return;
                }
                ...
                if (sur.Add_Str_InSql_2(Str_Classname, Str_Title, Str_TypeSelect, Str_MaxselectNum, Str_DisModel, Str_Starttime, Str_Endtime, Str_SortStyle, Str_isSteps, "0") != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "问卷调查系统新增主题成功!", "setTitle.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误.或许是没找到记录!", "setTitle.aspx");
                }
            }
        }
        ...
        protected void Editsave_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)//判断页面是否通过验证
            {
                int TID = int.Parse(Request.QueryString["ID"]);
                //取得修改中的表单信息
                string Str_ClassNameE = YSCMS.Common.Input.Filter(this.ClassnameE.Text.Trim());
                string Str_TitleE = YSCMS.Common.Input.Filter(this.TitleE.Text.Trim());
                string Str_TypeE = YSCMS.Common.Input.Filter(this.TypeE.Text.Trim());
                string Str_MaxNumE = YSCMS.Common.Input.Filter(this.MaxNumE.Text.Trim());
                string Str_DisModelE = YSCMS.Common.Input.Filter(this.DisModelE.Text.Trim());
                string Str_StartTimeE = YSCMS.Common.Input.Filter(this.StartTimeE.Text.Trim());
                string Str_EndTimeE = YSCMS.Common.Input.Filter(this.EndTimeE.Text.Trim());
                string Str_StyleE = YSCMS.Common.Input.Filter(this.StyleE.Text.Trim());
                string Str_isSteps = YSCMS.Common.Input.Filter(this.isStepsE.Text.Trim());
                ...
                //检查日期是否合法
                if (YSCMS.Common.Input.ChkDate(Str_StartTimeE) == false)
                {
                    YSCMS.Common.MessageBox.Show(this, "对不起，选项数只能为数字型，请返回继续添加!");
                    return;
                }
                if (YSCMS.Common.Input.ChkDate(Str_EndTimeE) == false)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，日期格式不正确，请返回继续添加!", "setTitle.aspx");
                }
                //刷新页面
                if (sur.Update_Str_UpdateSqls(Str_ClassNameE, Str_TitleE, Str_TypeE, Str_MaxNumE, Str_DisModelE, Str_StartTimeE, Str_EndTimeE, Str_StyleE, Str_isSteps, TID) != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "修改成功!", "setTitle.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.Show(this, "意外错误：未知错误,可能是没找到记录!");
                }
        ...
        protected void DelAll_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除全部成功!", "setTitle.aspx");
            }
        }
    }
}