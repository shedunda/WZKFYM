...
{
    public partial class SetClass : YSCMS.PageBasic.ManagePage
    {
        public SetClass()
        {
            Authority_Code = "S005";
        ...
        protected void SetClassEdit()
        {
            ...
            {
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "未知错误,异常错误", "SetClass.aspx");
            }
        }
        ...
        protected void setClassDel()
        {
            ...
            else
            {
                ...
                if (v1 && v2 && v3 && v4 && v5)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除成功!", "SetClass.aspx");
                }
                else
                {
                    rd.SaveUserAdminLogs(1, 1, UserNum, "删除投票分类", "删除数据失败");
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "SetClass.aspx");
                }
            }
        }
        ...
        protected void VoteClassManage(int PageIndex)//显示投票类别管理页面
        {
            ...
            string Keywrd = YSCMS.Common.Input.Filter(this.KeyWord.Text.Trim());
            ...
            if (Keywrd != "" && Keywrd != null)
            {
                ...
                {
                    case "choose":
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setClass_1_aspx", PageIndex, num, out i, out j, null);
                        break;
                    case "number":
                        SQLConditionInfo st = new SQLConditionInfo("@VID", "%" + Keywrd + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setClass_2_aspx", PageIndex, num, out i, out j, st);//按照id查询
                        break;
                    case "classname":
                        SQLConditionInfo sts = new SQLConditionInfo("@ClassName", "%" + Keywrd + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setClass_3_aspx", PageIndex, num, out i, out j, sts);//按照类名查询
                        break;
                    case "description":
                        SQLConditionInfo stss = new SQLConditionInfo("@Description", "%" + Keywrd + "%");
                        dt = YSCMS.CMS.Pagination.GetPage("manage_survey_setClass_4_aspx", PageIndex, num, out i, out j, stss);
                        break;
                    default:
                        break;
                }
        ...
        protected void SaveClass_ServerClick(object sender, EventArgs e)
        {
            ...
            {
                //取得投票参数设置添加中的表单信息
                string Str_ClassName = YSCMS.Common.Input.Filter(this.ClassName.Text.Trim());//类别名称
                string Str_Description = YSCMS.Common.Input.Filter(this.Description.Value.Trim());//描述

                //检查是否有已经存在的类别名称
                if (sur.sel_1(Str_ClassName) != 0)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，该类别已经存在", "SetClass.aspx?type=add");
                }
                //判断类别名称是否为空
                if (Str_ClassName == null || Str_ClassName == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，类别名称不能为空，请返回继续添加", "SetClass.aspx");
                }
                //向数据库中写入添加的类别信息
                ...
                if (sur.Add(Str_ClassName, Str_Description, SiteID) != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "问卷调查系统新增类别成功", "SetClass.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "意外错误：未知错误", "SetClass.aspx");
        ...
        protected void EditSave_ServerClick(object sender, EventArgs e)
        {
            if (Page.IsValid)//判断页面是否通过验证
            {
                int VID = int.Parse(Request.QueryString["ID"]);
                //取得添加中的表单信息
                string Str_ClassNameE = YSCMS.Common.Input.Filter(this.ClassNameEdit.Text.Trim());//类别名称
                string Str_DescriptionE = YSCMS.Common.Input.Filter(this.DescriptionE.Value.Trim());//描述

                //判断类别名称是否为空
                if (Str_ClassNameE == null || Str_ClassNameE == string.Empty)
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "对不起，类别名称不能为空，请返回继续修改!", "setClass.aspx");
                }
                ...
                if (sur.Update_Str_InSql(Str_ClassNameE, Str_DescriptionE, VID) != 0)
                {
                    ...
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "修改成功!", "setClass.aspx");
                }
                else
                {
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "修改成功!", "setClass.aspx");
        ...
        protected void DelAll_Click(object sender, EventArgs e)
        {
            ...
            if (v1 && v2 && v3 && v4)
            {
                ...
                YSCMS.Common.MessageBox.ShowAndRedirect(this, "删除全部成功!", "setClass.aspx");
            }
            else
            {
                YSCMS.Common.MessageBox.Show(this, "意外错误：未知错误");
