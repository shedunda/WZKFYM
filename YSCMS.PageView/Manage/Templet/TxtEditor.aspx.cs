...
{
    public partial class TxtEditor : YSCMS.PageBasic.ManagePage
    {
        public string dir = "";
        public string filename = "";
        private string str_dirMana = YSCMS.Config.UIConfig.dirDumm;
        private string str_Templet = YSCMS.Config.UIConfig.dirTemplet;  //获取模板路径
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)                                               //判断页面是否重载
            {
                ...
                {
                    string _sitePath = str_dirMana + "\\" + str_Templet + "\\siteTemplets\\" + YSCMS.Global.Current.SiteID;
        ...
        {
            FileContent.Text = YSCMS.Common.FileAction.ShowFileContet(filepath);
        }

        protected void getLabelList()
        {
            YSCMS.CMS.Label lb = new YSCMS.CMS.Label();
            DataTable dt = lb.getLableList(YSCMS.Global.Current.SiteID, 2);
            if (dt != null)
            {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                int result = 0;
                result = YSCMS.Common.FileAction.SaveFile(filepath, this.FileContent.Text);
                ...
                if (result == 1)
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "保存成功", url);
                else
                    YSCMS.Common.MessageBox.ShowAndRedirect(this, "参数错误", url);
            }
        }
    }
}